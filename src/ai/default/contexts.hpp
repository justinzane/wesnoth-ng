...
struct target {
        enum TYPE {
            VILLAGE, LEADER, EXPLICIT, THREAT, BATTLE_AID, MASS, SUPPORT
        };

        target(const map_location& pos, double val, TYPE target_type = VILLAGE) :
            loc(pos), value(val), type(target_type) {
        }
        map_location loc;
        double value;
...
class attack_analysis: public game_logic::formula_callable {
    public:
        ...
        void analyze(const gamemap& map,
                     unit_map& units,
...
class default_ai_context: public virtual readwrite_context {
    public:
        ...
        virtual int count_free_hexes_in_castle(const map_location& loc,
                                               std::set<map_location> &checked_hexes) = 0;
...
// proxies
class default_ai_context_proxy: public virtual default_ai_context,
    public virtual readwrite_context_proxy {
    public:
