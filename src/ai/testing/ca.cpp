...
bool recruitment_phase::recruit_usage(const std::string& usage)
{
	...
	foreach_ng(const std::string &name, current_team().recruits())
	{
...
double move_leader_to_goals_phase::evaluate()
{
	...
	map_location loc;
	foreach_ng(const map_location &l, route.steps)
	{
...
void move_leader_to_goals_phase::remove_goal(const std::string &id)
{
	...
	mgr::modify_active_ai_for_side(get_side(),mod_ai);
}
...
double move_leader_to_keep_phase::evaluate()
{
	...
	int next_hop_cost = 0;
	foreach_ng(const map_location& step, route.steps) {
	...
	foreach_ng(const ordered_locations::value_type& pair, moves_toward_keep) {
...
double retreat_phase::evaluate()
{
	...
	foreach_ng(unit_map::const_iterator leader, leaders){
		...
		foreach_ng(map_location &loc, tmp_leader_adj){
			bool found = false;
			foreach_ng(map_location &new_loc, leaders_adj_v){
				if(new_loc == loc){
					found = true;
					break;
				}
...
double leader_shares_keep_phase::evaluate()
{
	...
	foreach_ng(team &tmp_team, *resources::teams){
...
void leader_shares_keep_phase::execute()
{
	...
	foreach_ng(unit_map::unit_iterator &ai_leader, ai_leaders){
	...
	foreach_ng(unit_map::unit_iterator &leader, ai_leaders){
