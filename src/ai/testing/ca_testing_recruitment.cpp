/**
 * @file src/ai/testing/ca_testing_recruitment.cpp
...
class fake_team
{
   public:
      ...
      {
         int counter = 0;
         foreach_ng(unit &un, *resources::units){
...
/*static double get_unit_quality(const unit_type &info, fake_team &t, std::vector<fake_team> & fake_teams)
{
   ...
   foreach_ng(const unit &enemy_unit, *resources::units)
   {
   ...
   foreach_ng(fake_team &enemy_team, fake_teams)
   {
      if(!t.is_enemy(enemy_team.side()))
      {
         continue;
      }
      foreach_ng(const potential_recruit &enemy_unit,  enemy_team.extra_units())
      {
...
{
   foreach_ng( potential_recruit &recruit, t.recruit_list() )
   {
...
static std::vector<potential_recruit> ai_choose_best_recruits(fake_team &t, int max_units_to_recruit, double quality_factor, bool counter_recruit)
{
   ...
   foreach_ng(const potential_recruit &i, t.extra_units())
   {
      current_units[(i.id())]++;
   }
   ...
   foreach_ng(const potential_recruit &i, recruit_list)
   {
      if(i.cost() > max_cost)
      {
         max_cost = i.cost();
   ...
   int recruited = 0;
   foreach_ng(const potential_recruit *i, sorted)
   {
...
static void ai_choose_recruits(fake_team &t, int max_units_to_recruit, double quality_factor, bool counter_recruit)
{
   ...
   foreach_ng(potential_recruit &i, recruits) {
      t.fake_recruit(i);
   }
...
static void get_recruit_qualities(std::vector<potential_recruit> &recruit_list, fake_team &t, std::vector<fake_team> &fake_teams)
{
   ...
   foreach_ng(fake_team &tmp_t, fake_teams)
   {
	   if (t.is_enemy(tmp_t.side())) {
		   foreach_ng(potential_recruit &rec, tmp_t.extra_units())
		   {
   ...
   foreach_ng(potential_recruit &rec, recruit_list) {
	   double score = 0;
	   double weighting = 0;
	   foreach_ng(unit_map::value_type &enemy, enemies) {
...
//         const unit_type &enemy_type = *un.type();
         ...
         foreach_ng(unit &defender, *resources::units)
         {
      ...
      else
      {
         ...
         foreach_ng(unit &enemy, *resources::units)
         {
   ...
   foreach_ng(fake_team &tmp_t, fake_teams)
   {
      foreach_ng(potential_recruit &rec, tmp_t.extra_units()){
         if(t.is_enemy(tmp_t.side())){
            ...
            //HIER
            foreach_ng(fake_team &sub_t, fake_teams){
               if(t.is_enemy(sub_t.side())){
                  continue;
               }
               foreach_ng(potential_recruit &sub_rec, sub_t.extra_units()){
   ...
   double min_score = 0;
   double max_score = 0;
   foreach_ng(enemy_pair_type *pair, enemies)
   {
//      if(pair->defenders.size() == 0)
//      {
//         pair->score = -10000;
//      }else{
         //foreach_ng(unit_data *defender, pair->defenders)
         foreach_ng(unit_type *defender, pair->defenders)
         {
            unsigned int defender_enemies = 0;
            foreach_ng(defender_pair_type *defender_p, defenders)
            {
...
/*static double get_combat_score(fake_team &t, std::vector<fake_team> &fake_teams)
{
   ...
   double result = 0;
   foreach_ng(unit_map::value_type &defender, defenders)
   {
      double defenders_score = 0;
      foreach_ng(unit_map::value_type &enemy, enemies)
      {
         double hitpoints_sum = 0;
         foreach_ng(double i, enemy.second)
         {
            hitpoints_sum += i;
         }
      ...
      double hitpoints_sum = 0;
      foreach_ng(double i, defender.second)
      {
         hitpoints_sum += i;
      }
...
//   foreach_ng(enemy_pair &pair, enemies)
//   {
//      if(pair.defenders.size() == 0)
//      {
...
//            unsigned int defender_enemies = 0;
//            foreach_ng(defender_pair &defender_p, defenders)
//            {
//               if(defender.id == defender_p.defender.id){
//                  defender_enemies = defender_p.enemies.size();
//                  break;
...
//         //pair.score = 0;
//      }else{
//         foreach_ng(unit &defender, pair.defenders)
//         {
//            unsigned int defender_enemies = 0;
//            foreach_ng(defender_pair &defender_p, defenders)
//            {
...
void testing_recruitment_phase::do_recruit(int max_units_to_recruit, double quality_factor)
{
   ...
   for(int recruited_amount = 0; recruited_amount < max_units_to_recruit; recruited_amount++)
   {
      ...
      foreach_ng(fake_team &t, fake_teams)
      {
         t.reset();
      }
      ...
      foreach_ng(potential_recruit &recruit_type, ai_recruit_list)
      {
         foreach_ng(fake_team &t, fake_teams)
         {
            t.reset();
