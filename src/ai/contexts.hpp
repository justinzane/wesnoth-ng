...
struct defensive_position {
        defensive_position() :
            loc(), chance_to_hit(0), vulnerability(0.0), support(0.0) {
        }
...
class readonly_context: public virtual side_context {
    public:
        ...
        virtual bool is_active(const std::string &time_of_day,
                               const std::string &turns) const = 0;
         ...
         */
        virtual double power_projection(const map_location& loc,
                                        const move_map& dstsrc) const = 0;
        ...
        virtual const map_location& suitable_keep(const map_location& leader_location,
                                                  const pathfind::paths& leader_paths) = 0;
        ...
        typedef std::map<std::pair<map_location, const unit_type *>,
            std::pair<battle_context_unit_stats, battle_context_unit_stats> > unit_stats_cache_t;
...
class readwrite_context: public virtual readonly_context {
    public:
        ...
        virtual attack_result_ptr execute_attack_action(const map_location& attacker_loc,
                                                        const map_location& defender_loc,
                                                        int attacker_weapon) = 0;
        ...
        virtual stopunit_result_ptr execute_stopunit_action(const map_location& unit_location,
                                                            bool remove_movement = true,
                                                            bool remove_attacks = false) = 0;
...
class readonly_context_proxy: public virtual readonly_context, public virtual side_context_proxy {
    public:
        ...
        virtual stopunit_result_ptr check_stopunit_action(const map_location &unit_location,
            ...
            return target_->check_stopunit_action(unit_location, remove_movement,
                                                  remove_attacks);
        }
        ...
        virtual void calculate_possible_moves(std::map<map_location, pathfind::paths>& possible_moves,
                                              ...
                                              const terrain_filter* remove_destinations =
                                                  nullptr) const {
            target_->calculate_possible_moves(possible_moves, srcdst, dstsrc, enemy,
                                              assume_full_movement, remove_destinations);
        }
        ...
        virtual void calculate_moves(const unit_map& units,
            ...
            target_->calculate_moves(units, possible_moves, srcdst, dstsrc, enemy,
                                     assume_full_movement, remove_destinations, see_all);
        }
...
class readwrite_context_proxy: public virtual readwrite_context,
    public virtual readonly_context_proxy {
    public:
        ...
        virtual stopunit_result_ptr execute_stopunit_action(const map_location& unit_location,
                                                            bool remove_movement = true,
                                                            bool remove_attacks = false) {
            return target_->execute_stopunit_action(unit_location, remove_movement,
                                                    remove_attacks);
        }
...
class readonly_context_impl:
    public virtual side_context_proxy,
    public readonly_context,
    public evt_observer {
    public:
         ...
         */
        attack_result_ptr check_attack_action(const map_location& attacker_loc,
                                              const map_location& defender_loc,
                                              int attacker_weapon);
         ...
         */
        recall_result_ptr check_recall_action(const std::string& id, const map_location &where =
            map_location::null_location,
                                              const map_location &from =
                                                  map_location::null_location);
         ...
         */
        stopunit_result_ptr check_stopunit_action(const map_location& unit_location,
                                                  bool remove_movement = true,
                                                  bool remove_attacks = false);
    ...
    private:
        template <typename T>
        void add_known_aspect(const std::string &name,
