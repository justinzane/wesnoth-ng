...
void readonly_context_impl::on_readonly_context_create() {
	...
	foreach_ng(const config &cfg_element, cfg_.child_range("engine")){
		engine::parse_engine_from_config(*this,cfg_element,std::back_inserter(engines_));
	}

	// init the composite ai aspects
	foreach_ng(const config &cfg_element, cfg_.child_range("aspect")){
	...
	foreach_ng(const config &cfg_element, cfg_.child_range("goal")){
...
{
	config cfg;
	foreach_ng(const engine_ptr e, engines_) {
		cfg.add_child("engine",e->to_config());
	}
	foreach_ng(const aspect_map::value_type a, aspects_) {
		cfg.add_child("aspect",a.second->to_config());
	}
	foreach_ng(const goal_ptr g, goals_) {
...
{
	if(game_config::debug) {
		resources::screen->add_chat_message(time(nullptr), "ai", get_side(), msg,
          ...
          ) const
{
	...
	for(std::map<map_location,pathfind::paths>::iterator m = res.begin(); m != res.end(); ++m) {
		foreach_ng(const pathfind::paths::step &dest, m->second.destinations)
		{
...
{
	foreach_ng(aspect_ptr a, aspects) {
...
double readonly_context_impl::power_projection(const map_location& loc, const move_map& dstsrc) const
{
	...
	for (int i = 0;; ++i) {
		...
		for(Itor it = its.first; it != its.second; ++it) {
			...
			int most_damage = 0;
			foreach_ng(const attack_type &att, un.attacks())
			{
...
const map_location& readonly_context_impl::suitable_keep(const map_location& leader_location, const pathfind::paths& leader_paths){
	...
	foreach_ng(const pathfind::paths::step &dest, leader_paths.destinations)
	{
