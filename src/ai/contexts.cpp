...
attack_result_ptr readwrite_context_impl::execute_attack_action(const map_location& attacker_loc,
                                                                const map_location& defender_loc,
                                                                int attacker_weapon) {
    unit_map::iterator i = resources::units->find(attacker_loc);
    double m_aggression =
        i.valid() && i->can_recruit() ? get_leader_aggression() : get_aggression();
    const unit_advancements_aspect& m_advancements = get_advancements();
    return actions::execute_attack_action(get_side(), true, attacker_loc, defender_loc,
                                          attacker_weapon, m_aggression, m_advancements);
}

attack_result_ptr readonly_context_impl::check_attack_action(const map_location& attacker_loc,
                                                             const map_location& defender_loc,
                                                             int attacker_weapon) {
    unit_map::iterator i = resources::units->find(attacker_loc);
    double m_aggression =
        i.valid() && i->can_recruit() ? get_leader_aggression() : get_aggression();
    const unit_advancements_aspect& m_advancements = get_advancements();
    return actions::execute_attack_action(get_side(), false, attacker_loc, defender_loc,
                                          attacker_weapon, m_aggression, m_advancements);
}

move_result_ptr readwrite_context_impl::execute_move_action(const map_location& from,
...
move_result_ptr readonly_context_impl::check_move_action(const map_location& from,
                                                         const map_location& to,
                                                         bool remove_movement,
                                                         bool unreach_is_ok) {
...
recall_result_ptr readwrite_context_impl::execute_recall_action(const std::string& id,
                                                                const map_location &where,
                                                                const map_location &from) {
    return actions::execute_recall_action(get_side(), true, id, where, from);
}

recruit_result_ptr readwrite_context_impl::execute_recruit_action(const std::string& unit_name,
                                                                  const map_location &where,
                                                                  const map_location &from) {
    return actions::execute_recruit_action(get_side(), true, unit_name, where, from);
}

recall_result_ptr readonly_context_impl::check_recall_action(const std::string& id,
                                                             const map_location &where,
                                                             const map_location &from) {
    return actions::execute_recall_action(get_side(), false, id, where, from);
}

recruit_result_ptr readonly_context_impl::check_recruit_action(const std::string& unit_name,
                                                               const map_location &where,
                                                               const map_location &from) {
    return actions::execute_recruit_action(get_side(), false, unit_name, where, from);
}

stopunit_result_ptr readwrite_context_impl::execute_stopunit_action(const map_location& unit_location,
                                                                    bool remove_movement,
                                                                    bool remove_attacks) {
    return actions::execute_stopunit_action(get_side(), true, unit_location, remove_movement,
                                            remove_attacks);
}

stopunit_result_ptr readonly_context_impl::check_stopunit_action(const map_location& unit_location,
                                                                 bool remove_movement,
                                                                 bool remove_attacks) {
    return actions::execute_stopunit_action(get_side(), false, unit_location, remove_movement,
                                            remove_attacks);
}

template <typename T>
void readonly_context_impl::add_known_aspect(const std::string &name,
                                             boost::shared_ptr<typesafe_aspect<T> > &where) {
    boost::shared_ptr<typesafe_known_aspect<T> > ka_ptr(
        new typesafe_known_aspect<T>(name, where, aspects_));
...
void readonly_context_impl::on_readonly_context_create() {
    ...
    BOOST_FOREACH(const config &cfg_element, cfg_.child_range("aspect")) {
        ...
        engine::parse_aspect_from_config( *this, cfg_element, cfg_element["id"],
                                         std::back_inserter(aspects));
        add_aspects(aspects);
    }
...
void readonly_context_impl::calculate_possible_moves(std::map<map_location, pathfind::paths>& res,
    ...
    calculate_moves( *resources::units, res, srcdst, dstsrc, enemy, assume_full_movement,
                    remove_destinations);
}
...
void readonly_context_impl::calculate_moves(const unit_map& units,
    ...
    for (unit_map::const_iterator un_it = units.begin(); un_it != units.end(); ++un_it) {
        ...
        if ( (enemy && current_team().is_enemy(un_it->side()) == false) || ( !enemy
            && !assume_full_movement && un_it->side() != get_side())
        ...
        if (un_it->movement_left() > 0) {
            std::pair<map_location, map_location> trivial_mv(un_it->get_location(),
                                                             un_it->get_location());
         ...
         */
        res.insert(
            std::pair<map_location, pathfind::paths>(
                un_it->get_location(),
                pathfind::paths( *un_it, false, true, current_team(), 0, see_all)));
    }
    ...
    for (std::map<map_location, pathfind::paths>::iterator m = res.begin(); m != res.end();
        ++m) {
...
const defensive_position& readonly_context_impl::best_defensive_position(const map_location& loc,
    ...
    for (Itor i = itors.first; i != itors.second; ++i) {
        ...
        if (defense < pos.chance_to_hit || support - vulnerability
            > pos.support - pos.vulnerability) {
            pos.loc = i->second;
...
double readonly_context_impl::power_projection(const map_location& loc,
    ...
    for (int i = 0;; ++i) {
        ...
        for (Itor it = its.first; it != its.second; ++it) {
            ...
            const int lawful_bonus = resources::tod_manager->get_time_of_day(attack_turn)
                .lawful_bonus;
            int tod_modifier = 0;
...
const map_location& readonly_context_impl::suitable_keep(const map_location& leader_location,
    ...
    BOOST_FOREACH(const pathfind::paths::step &dest, leader_paths.destinations) {
        ...
        if (keeps().find(loc) != keeps().end()) {
            ...
            if (resources::units->count(loc) == 0) {
                if ( ( *best_free_keep == map_location::null_location) || (move_left_at_loc
                    > move_left_at_best_free_keep)) {
                    best_free_keep = &loc;
                    move_left_at_best_free_keep = move_left_at_loc;
                }
            } else {
                if ( ( *best_occupied_keep == map_location::null_location) || (move_left_at_loc
...
bool readonly_context_impl::is_active(const std::string &time_of_day,
                                      const std::string &turns) const {
    ...
    if (turns.empty() == false) {
        ...
        for (std::vector<std::string>::const_iterator j = turns_list.begin();
            j != turns_list.end(); ++j) {
