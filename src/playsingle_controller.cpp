...
playsingle_controller::playsingle_controller(const config& level,
    ...
    if (state_of_game.classification().completion == "victory" ||
        state_of_game.classification().completion == "defeat") {
...
void playsingle_controller::repeat_recruit() {
    if ( !browse_)
        ...
        menu_handler_.repeat_recruit(resources::screen->viewing_side(),
...
void playsingle_controller::unit_hold_position() {
    if ( !browse_) menu_handler_.unit_hold_position(mouse_handler_, player_number_);
}

void playsingle_controller::end_unit_turn() {
    if ( !browse_) menu_handler_.end_unit_turn(mouse_handler_, player_number_);
}
...
void playsingle_controller::report_victory(std::ostringstream &report,
                                           int player_gold,
                                           int remaining_gold,
    ...
    report << _("Remaining gold: ") << utils::half_signed_value(remaining_gold) << "\n";
    if (end_level.gold_bonus) {
        if (turns_left > -1) {
            report << _("Early finish bonus: ") << finishing_bonus_per_turn << " "
                   << _("per turn") << "\n" << "<b>" << _("Turns finished early: ")
                   << turns_left << "</b>\n" << _("Bonus: ") << finishing_bonus << "\n";
        }
        report << _("Gold: ") << utils::half_signed_value(remaining_gold + finishing_bonus);
    }
    if (remaining_gold > 0) {
        report << '\n' << _("Carry over percentage: ") << end_level.carryover_percentage;
    }
    if (end_level.carryover_add) {
        report << "\n<b>" << _("Bonus Gold: ") << utils::half_signed_value(player_gold)
               << "</b>";
    } else {
        report << "\n<b>" << _("Retained Gold: ") << utils::half_signed_value(player_gold)
               << "</b>";
    }
    ...
    if (end_level.carryover_add) {
        if (player_gold > 0) {
            goldmsg = vngettext("You will start the next scenario with $gold "
            ...
            goldmsg = vngettext("You will start the next scenario with "
...
LEVEL_RESULT playsingle_controller::play_scenario(const config::const_child_itors &story,
                                                  bool skip_replay) {
...
void playsingle_controller::play_turn(bool save) {
    ...
    if (non_interactive()) std::cout << "Turn " << turn() << ":" << std::endl;

    for (player_number_ = first_player_; player_number_ <= int(teams_.size());
        ++player_number_) {
        ...
        if (non_interactive()) {
            std::cout << " Player " << player_number_ << ": "
                      << current_team().villages().size() << " Villages" << std::endl;
    ...
    if (player_number_ > static_cast<int>(teams_.size())) player_number_ = teams_.size();
...
void playsingle_controller::play_side(const unsigned int side_number, bool save) {
    ...
    do {
        ...
        player_type_changed_ = false;
        if ( !skip_next_turn_) end_turn_ = false;
...
void playsingle_controller::before_human_turn(bool save) {
    ...
    if (save && level_result_ == NONE) {
        savegame::autosave_savegame save(gamestate_, *gui_, to_config(),
                                         preferences::compress_saves());
        save.autosave(game_config::disable_autosave, preferences::autosavemax(),
...
hotkey::ACTION_STATE playsingle_controller::get_action_state(hotkey::HOTKEY_COMMAND command,
                                                             int index) const {
    switch (command) {
...
bool playsingle_controller::can_execute_command(const hotkey::hotkey_command& cmd,
                                                int index) const {
    ...
    switch (command) {
        ...
        case hotkey::HOTKEY_WML:
            ...
            return (gui_->viewing_team() == gui_->playing_team()) && !events::commands_disabled
                   && teams_[gui_->viewing_team()].is_human() && !linger_ && !browse_;
        ...
        case hotkey::HOTKEY_RECALL:
            return ( !browse_ || resources::whiteboard->is_active()) && !linger_
        ...
        case hotkey::HOTKEY_DELAY_SHROUD:
            return !linger_
                && (teams_[gui_->viewing_team()].uses_fog() || teams_[gui_->viewing_team()]
                    .uses_shroud())
                && !events::commands_disabled;
        case hotkey::HOTKEY_UPDATE_SHROUD:
            return !linger_ && player_number_ == gui_->viewing_side()
                   && !events::commands_disabled
        ...
        case hotkey::HOTKEY_LABEL_TERRAIN: {
            const terrain_label *label = resources::screen->labels().get_label(
                mouse_handler_.get_last_hex());
            res = !events::commands_disabled && map_.on_board(mouse_handler_.get_last_hex())
                  && !gui_->shrouded(mouse_handler_.get_last_hex()) && !is_observer()
                  && ( !label || !label->immutable());
            break;
        }
        case hotkey::HOTKEY_CONTINUE_MOVE: {
            if (browse_ || events::commands_disabled) return false;

            if ( (menu_handler_.current_unit() != units_.end()) && (menu_handler_.current_unit()
                ->move_interrupted())) return true;
