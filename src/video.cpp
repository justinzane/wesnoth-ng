...
void resize_monitor::process(events::pump_info &info) {
    if (info.resize_dimensions.first >= preferences::min_allowed_width() && info
        .resize_dimensions.second
...
void CVideo::initSDL() {
    ...
    window_ = SDL_CreateWindow("WesnothNG", 0, 0, 0, 0,
                               (SDL_WINDOW_RESIZABLE |
                                SDL_WINDOW_MAXIMIZED |
                                SDL_WINDOW_OPENGL));
}

void CVideo::blit_surface(int x, int y, SDL_Surface surf, SDL_Rect* srcrect, SDL_Rect* clip_rect) {
...
void CVideo::flip() {
    /** TODO Fix this with RenderClear, Render.... */
//    if (update_all) {
...
bool CVideo::isFullScreen() const {
    return ((SDL_GetWindowFlags(window_) & SDL_WINDOW_FULLSCREEN) ||
            (SDL_GetWindowFlags(window_) & SDL_WINDOW_FULLSCREEN_DESKTOP));
}
