/**
 * @file src/side_filter.cpp
 * @project The Battle for Wesnoth NG - https://github.com/justinzane/wesnoth-ng
 * @brief 
 * @authors 
 * @copyright Copyright (C) 2010 - 2013 by Yurii Chernyi <terraninfo@terraninfo.net>
...
std::vector<int> side_filter::get_teams() const
{
	...
	foreach_ng(const team &t, *resources::teams) {
		if (match(t)) {
...
bool side_filter::match_internal(const team &t) const
{
	...
	if (!cfg_team_name.blank()) {
		...
		if(std::find(this_team_name.begin(), this_team_name.end(), ',') == this_team_name.end()) {
			...
			foreach_ng(const std::string& this_single_team_name, these_team_names) {
	...
	if(cfg_.has_child("has_unit")) {
		...
		if(!found && unit_filter["search_recall_list"].to_bool(false)) {
			...
			foreach_ng(const unit& u, recall_list) {
	...
	if(!enemy_of.null()) {
		...
		foreach_ng(const int side, teams) {
