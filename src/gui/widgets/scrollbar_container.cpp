...
static const std::string button_down_names[] = { "_end", "_line_down", "_half_page_down",
                                                 "_page_down" };
...
tscrollbar_container::tscrollbar_container(const unsigned canvas_count) :
    ...
    connect_signal<event::SDL_KEY_DOWN>(
        boost::bind( &tscrollbar_container::signal_handler_sdl_key_down, this, _2, _3, _5, _6));

    connect_signal<event::SDL_WHEEL_UP>(
        boost::bind( &tscrollbar_container::signal_handler_sdl_wheel_up, this, _2, _3),
        event::tdispatcher::back_post_child);

    connect_signal<event::SDL_WHEEL_DOWN>(
        boost::bind( &tscrollbar_container::signal_handler_sdl_wheel_down, this, _2, _3),
        event::tdispatcher::back_post_child);

    connect_signal<event::SDL_WHEEL_LEFT>(
        boost::bind( &tscrollbar_container::signal_handler_sdl_wheel_left, this, _2, _3),
        event::tdispatcher::back_post_child);

    connect_signal<event::SDL_WHEEL_RIGHT>(
        boost::bind( &tscrollbar_container::signal_handler_sdl_wheel_right, this, _2, _3),
        event::tdispatcher::back_post_child);
}
...
void tscrollbar_container::layout_initialise(const bool full_initialisation) {
    ...
    if (full_initialisation) {
        ...
        switch (vertical_scrollbar_mode_) {
            ...
            case auto_visible:
                vertical_scrollbar_grid_->set_visible(twidget::tvisible::hidden);
                break;
        ...
        switch (horizontal_scrollbar_mode_) {
            ...
            case auto_visible:
                horizontal_scrollbar_grid_->set_visible(twidget::tvisible::hidden);
                break;
...
tpoint tscrollbar_container::calculate_best_size() const {
    ...
    const tpoint vertical_scrollbar =
        vertical_scrollbar_grid_->get_visible() == twidget::tvisible::invisible ?
            tpoint(0, 0) : vertical_scrollbar_grid_->get_best_size();

    /***** get horizontal scrollbar size *****/
    const tpoint horizontal_scrollbar =
        horizontal_scrollbar_grid_->get_visible() == twidget::tvisible::invisible ?
            tpoint(0, 0) : horizontal_scrollbar_grid_->get_best_size();

    /***** get content size *****/
    assert(content_grid_);
    const tpoint content = content_grid_->get_best_size();

    const tpoint result(vertical_scrollbar.x + std::max(horizontal_scrollbar.x, content.x),
...
static void set_scrollbar_mode(tgrid* scrollbar_grid,
    ...
    if (scrollbar_mode == tscrollbar_container::auto_visible) {
        ...
        scrollbar_grid->set_visible(
            scrollbar_needed ? twidget::tvisible::visible : twidget::tvisible::hidden);
    }
}
...
void tscrollbar_container::place(const tpoint& origin, const tpoint& size) {
    ...
    set_scrollbar_mode(vertical_scrollbar_grid_, vertical_scrollbar_, vertical_scrollbar_mode_,
                       content_grid_->get_height(), content_->get_height());

    // Set horizontal scrollbar
    set_scrollbar_mode(horizontal_scrollbar_grid_, horizontal_scrollbar_,
                       horizontal_scrollbar_mode_, content_grid_->get_width(),
                       content_->get_width());
...
void tscrollbar_container::set_visible_rectangle(const SDL_Rect& rectangle) {
    ...
    content_visible_area_ = intersect_rects(rectangle, content_->get_rectangle());
...
twidget* tscrollbar_container::find_at(const tpoint& coordinate, const bool must_be_active) {
    return tscrollbar_container_implementation::find_at<twidget>( *this, coordinate,
                                                                 must_be_active);
}

const twidget* tscrollbar_container::find_at(const tpoint& coordinate,
                                             const bool must_be_active) const {
    return tscrollbar_container_implementation::find_at<const twidget>( *this, coordinate,
                                                                       must_be_active);
}

twidget* tscrollbar_container::find(const std::string& id, const bool must_be_active) {
    return tscrollbar_container_implementation::find<twidget>( *this, id, must_be_active);
}

const twidget* tscrollbar_container::find(const std::string& id,
                                          const bool must_be_active) const {
    return tscrollbar_container_implementation::find<const twidget>( *this, id, must_be_active);
}
...
bool tscrollbar_container::content_resize_width(const int width_modification) {
    ...
    if (horizontal_scrollbar_mode_ == always_invisible || (horizontal_scrollbar_mode_
    ...
    set_scrollbar_mode(horizontal_scrollbar_grid_, horizontal_scrollbar_,
                       horizontal_scrollbar_mode_, new_width, content_->get_width());
...
bool tscrollbar_container::content_resize_height(const int height_modification) {
    ...
    if (vertical_scrollbar_mode_ == always_invisible || (vertical_scrollbar_mode_
        == auto_visible_first_run
                                                         && vertical_scrollbar_grid_
                                                             ->get_visible()
                                                            == twidget::tvisible::invisible)) {
    ...
    set_scrollbar_mode(vertical_scrollbar_grid_, vertical_scrollbar_, vertical_scrollbar_mode_,
                       new_height, content_->get_height());
...
void tscrollbar_container::finalize_setup() {
    ...
    vertical_scrollbar_grid_ = find_widget<tgrid>(this, "_vertical_scrollbar_grid", false,
                                                  true);

    vertical_scrollbar_ = find_widget<tscrollbar_>(vertical_scrollbar_grid_,
                                                   "_vertical_scrollbar", false, true);
    ...
    horizontal_scrollbar_grid_ = find_widget<tgrid>(this, "_horizontal_scrollbar_grid", false,
                                                    true);

    horizontal_scrollbar_ = find_widget<tscrollbar_>(horizontal_scrollbar_grid_,
                                                     "_horizontal_scrollbar", false, true);

    connect_signal_notify_modified(
        *horizontal_scrollbar_,
        boost::bind( &tscrollbar_container::horizontal_scrollbar_moved, this));

    /***** Setup the scrollbar buttons *****/
    FOREACH(const AUTO& item, scroll_lookup()) {

        // Vertical.
        tclickable_* button = find_widget<tclickable_>(vertical_scrollbar_grid_, item.first,
                                                       false, false);

        if (button) {
            button->connect_click_handler(
                boost::bind( &tscrollbar_container::scroll_vertical_scrollbar, this,
                            item.second));
        }

        // Horizontal.
        button = find_widget<tclickable_>(horizontal_scrollbar_grid_, item.first, false, false);

        if (button) {
            button->connect_click_handler(
                boost::bind( &tscrollbar_container::scroll_horizontal_scrollbar, this,
                            item.second));
        }
    }
    ...
    content_grid_ = dynamic_cast<tgrid*>(grid().swap_child("_content_grid", content_, true));
    assert(content_grid_);
...
void tscrollbar_container::set_vertical_scrollbar_mode(const tscrollbar_mode scrollbar_mode) {
    if (vertical_scrollbar_mode_ != scrollbar_mode) {
        vertical_scrollbar_mode_ = scrollbar_mode;
    }
}

void tscrollbar_container::set_horizontal_scrollbar_mode(const tscrollbar_mode scrollbar_mode) {
...
void tscrollbar_container::impl_draw_children(surface& frame_buffer) {
    assert(
        get_visible() == twidget::tvisible::visible && content_grid_->get_visible()
...
void tscrollbar_container::set_scrollbar_button_status() {
    if (true) { /** @todo scrollbar visibility. */
        ...
        FOREACH(const AUTO& name, button_up_names) {
            tcontrol* button = find_widget<tcontrol>(vertical_scrollbar_grid_, name, false,
                                                     false);
    ...
    if (true) { /** @todo scrollbar visibility. */
        ...
        FOREACH(const AUTO& name, button_up_names) {
            tcontrol* button = find_widget<tcontrol>(horizontal_scrollbar_grid_, name, false,
                                                     false);
...
void tscrollbar_container::scroll_vertical_scrollbar(const tscrollbar_::tscroll scroll) {
...
void tscrollbar_container::scroll_horizontal_scrollbar(const tscrollbar_::tscroll scroll) {
...
void tscrollbar_container::handle_key_page_up(SDL_Keymod /*modifier*/, bool& handled) {
    assert(vertical_scrollbar_);
...
void tscrollbar_container::handle_key_page_down(SDL_Keymod /*modifier*/, bool& handled)
...
void tscrollbar_container::handle_key_up_arrow(SDL_Keymod /*modifier*/, bool& handled) {
    assert(vertical_scrollbar_);
...
void tscrollbar_container::handle_key_down_arrow(SDL_Keymod /*modifier*/, bool& handled) {
    assert(vertical_scrollbar_);
...
void tscrollbar_container::handle_key_left_arrow(SDL_Keymod /*modifier*/, bool& handled) {
    assert(horizontal_scrollbar_);
...
void tscrollbar_container::handle_key_right_arrow(SDL_Keymod /*modifier*/, bool& handled) {
    assert(horizontal_scrollbar_);
...
void tscrollbar_container::scrollbar_moved() {
    ...
    const int y_offset =
        vertical_scrollbar_mode_ == always_invisible ?
            0 : vertical_scrollbar_->get_item_position() * vertical_scrollbar_->get_step_size();
...
void tscrollbar_container::signal_handler_sdl_key_down(const event::tevent event,
                                                       bool& handled,
                                                       const SDLKey key,
                                                       SDL_Keymod modifier) {
...
void tscrollbar_container::signal_handler_sdl_wheel_up(const event::tevent event,
                                                       bool& handled) {
    DBG_GUI_E<< LOG_HEADER << event << ".\n";
...
void tscrollbar_container::signal_handler_sdl_wheel_down(const event::tevent event,
                                                         bool& handled) {
    DBG_GUI_E<< LOG_HEADER << event << ".\n";
...
void tscrollbar_container::signal_handler_sdl_wheel_left(const event::tevent event,
                                                         bool& handled) {
    DBG_GUI_E<< LOG_HEADER << event << ".\n";
...
void tscrollbar_container::signal_handler_sdl_wheel_right(const event::tevent event,
                                                          bool& handled) {
    DBG_GUI_E<<LOG_HEADER << event << ".\n";
