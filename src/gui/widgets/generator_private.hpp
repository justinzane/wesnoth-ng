...
#include "../auxiliary/iterator/walker.hpp"
...
struct tvertical_list: public virtual tgenerator_ {
        ...
        /** Inherited from tgenerator_. */
        void handle_key_left_arrow(SDL_keymod /*modifier*/, bool& /*handled*/) { /* do nothing */
        }

        /** Inherited from tgenerator_. */
        void handle_key_right_arrow(SDL_keymod /*modifier*/, bool& /*handled*/) { /* do nothing */
        }
...
struct tmatrix: public virtual tgenerator_ {
        ...
        /** Inherited from tgenerator_. */
        void handle_key_up_arrow(SDL_keymod, bool&) {
            ERROR_LOG(false);
        }

        /** Inherited from tgenerator_. */
        void handle_key_down_arrow(SDL_keymod, bool&) {
            ERROR_LOG(false);
        }

        /** Inherited from tgenerator_. */
        void handle_key_left_arrow(SDL_keymod, bool&) {
            ERROR_LOG(false);
        }

        /** Inherited from tgenerator_. */
        void handle_key_right_arrow(SDL_keymod, bool&) {
            ERROR_LOG(false);
        }
};
...
struct tshow: public virtual tgenerator_ {
        void select(tgrid& grid, const bool show) {
            grid.set_visible(show ? twidget::tvisible::visible : twidget::tvisible::hidden);
        }
...
class tgenerator: public minimum_selection,
    ...
    public:
        ...
        void set_item_shown(const unsigned index, const bool show) {
            ...
            if (items_[index]->shown != show) {
                ...
                items_[index]->grid.set_visible(
                    show ? twidget::tvisible::visible : twidget::tvisible::invisible);
        ...
        virtual void create_items(const int index,
                                  tbuilder_grid_const_ptr list_builder,
                                  const std::vector<
                                      std::map<std::string /*widget id*/, string_map> >& data,
                                  const boost::function<void(twidget&)>& callback) {
        ...
        {
            FOREACH(AUTO item, items_) {
                if (item->grid.get_visible() != twidget::tvisible::invisible && item->shown) {
        ...
        virtual void impl_draw_children(surface& frame_buffer) OVERRIDE
        {
            ...
            FOREACH(AUTO item, items_) {
                if (item->grid.get_visible() == twidget::tvisible::visible && item->shown) {
        ...
        /** See @ref twidget::impl_draw_children. */
        virtual void impl_draw_children(surface& frame_buffer, int x_offset, int y_offset)
            OVERRIDE
            {
            assert(this->get_visible() == twidget::tvisible::visible);
    ...
    private:
        ...
        struct titem {
                ...
                titem() :
                    grid(), selected(false), shown(true) {
                }
