...
tstacked_widget::tstacked_widget() :
    tcontainer_(1),
    generator_(tgenerator_::build(false, false, tgenerator_::independent, false)) {
}
 ...
 * @param id
 */
void swap_grid(tgrid* grid, tgrid* content_grid, twidget* widget, const std::string& id) {
    if (!content_grid) { throw std::invalid_argument("Cannot swap a void grid."); }
    if (!widget) { throw std::invalid_argument("Cannot swap_grid with void widget."); }
    ...
    if (!parent_grid) { throw std::logic_error("Could not find parent_grid."); }

    // Replace the child.
    widget = parent_grid->swap_child(id, widget, false);
    if (!widget) { throw std::invalid_argument("swap_child returned void widget."); }

    /** @todo explain why this is swap_grid not move_grid since we delete the widget */
    delete widget;
}
