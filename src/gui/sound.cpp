...
bool init_sound() {
	...
	if(SDL_GetCurrentAudioDriver()) {
	    // We have a current audio driver/
	    LOG_AUDIO << "Current SDL Audio Driver: " << SDL_GetCurrentAudioDriver() << "\n";
	    return true;
	} else {
	    // Try to initialize anything.
	    /// @todo make me use preferred drivers
	    for (int i = 0; i < SDL_GetNumAudioDrivers(); ++i) {
	        const char* driver_name = SDL_GetAudioDriver(i);
	        if (SDL_AudioInit(driver_name)) {
	            LOG_AUDIO << "Audio driver failed to initialize: " << driver_name << "\n";
	        } else {
	            LOG_AUDIO << "Current SDL Audio Driver: " << SDL_GetCurrentAudioDriver() << "\n";
	            return true;
	        }
