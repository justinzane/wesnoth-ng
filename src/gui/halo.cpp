...
class effect {
    ...
    private:
        ...
        halo_orientation_t orientation_;
...
bool effect::render() {
    ...
    if (loc_.x != -1 && loc_.y != -1) {
        if (disp->shrouded(loc_)) {
            ...
            set_location(disp->get_location_x(loc_) + disp->hex_size() / 2,
    ...
    if (orientation_ == HORIZ_REFLECT || orientation_ == BOTH_REFLECT) {
        surf_.assign(mirror_surface(surf_, HORIZ));
    }
    if (orientation_ == VERT_REFLECT || orientation_ == BOTH_REFLECT) {
...
halo_mgr::halo_mgr(display& screen) :
    old(disp) {
    disp = &screen;
}
...
int halo_add(int x,
    ...
    for (; itor != items.end(); ++itor) {
        ...
        image_vector.push_back(
            animated<image::locator>::frame_description(time, image::locator(str)));

    }
    halos.insert(
        std::pair<int, effect>(id, effect(x, y, image_vector, loc, orientation, infinite)));
    new_halos.insert(id);
    if (halos.find(id)->second.does_change() || !infinite) {
...
void halo_set_loc(int handle, int x, int y) {
    const std::map<int, effect>::iterator itor = halos.find(handle);
    if (itor != halos.end()) {
...
void halo_remove(int handle) {
    ...
    if (handle == HALO_INVALID || halos.find(handle) == halos.end()) {
        return;
    }

    deleted_halos.insert(handle);
}

void halo_unrender(std::set<map_location> invalidated_locations) {
    if (preferences::show_haloes() == false || halos.size() == 0) {
        return;
    }
    //assert(invalidated_haloes.size() == 0);

    // Remove expired haloes
    std::map<int, effect>::iterator itor = halos.begin();
    for (; itor != halos.end(); ++itor) {
        if (itor->second.expired()) {
            deleted_halos.insert(itor->first);
        }
    }

    // Add the haloes marked for deletion to the invalidation set
    std::set<int>::const_iterator set_itor = deleted_halos.begin();
    for (; set_itor != deleted_halos.end(); ++set_itor) {
        invalidated_halos.insert( *set_itor);
        halos.find( *set_itor)->second.add_overlay_location(invalidated_locations);
    }
    ...
    for (set_itor = changing_haloes.begin(); set_itor != changing_haloes.end(); ++set_itor) {
        if (halos.find( *set_itor)->second.need_update()) {
            invalidated_halos.insert( *set_itor);
            halos.find( *set_itor)->second.add_overlay_location(invalidated_locations);
        }
    }
    ...
    do {
        halo_count = invalidated_halos.size();
        for (itor = halos.begin(); itor != halos.end(); ++itor) {
            // Test all haloes not yet in the set
            // which match one of the locations
            if (invalidated_halos.find(itor->first) == invalidated_halos.end() && itor->second
                .on_location(invalidated_locations)) {

                // If found, add all locations which the halo invalidates,
                // and add it to the set
                itor->second.add_overlay_location(invalidated_locations);
                invalidated_halos.insert(itor->first);
            }
        }
    } while (halo_count != invalidated_halos.size() && halo_count != halos.size());
    ...
    for (std::map<int, effect>::reverse_iterator ritor = halos.rbegin();
        ritor != halos.rend(); ++ritor) {
        if (invalidated_halos.find(ritor->first) != invalidated_halos.end()) {
            ritor->second.unrender();
        }
    }

    // Really delete the haloes marked for deletion
    for (set_itor = deleted_halos.begin(); set_itor != deleted_halos.end(); ++set_itor) {
        // It can happen a deleted halo hasn't been rendered yet, invalidate them as well
        new_halos.erase( *set_itor);

        changing_haloes.erase( *set_itor);
        invalidated_halos.erase( *set_itor);
        halos.erase( *set_itor);
    }
...
void halo_render() {
    if (preferences::show_haloes() == false || halos.size() == 0
        || (new_halos.size() == 0 && invalidated_halos.size() == 0)) {
        return;
    }
    ...
    for (std::map<int, effect>::iterator itor = halos.begin(); itor != halos.end(); ++itor) {

        if (new_halos.find(itor->first) != new_halos.end() && !itor->second.render()) {

            unrendered_new_haloes.insert(itor->first);
        } else if (invalidated_halos.find(itor->first) != invalidated_halos.end()) {
            itor->second.render();
        }
    }

    invalidated_halos.clear();
    new_halos = unrendered_new_haloes;
}
