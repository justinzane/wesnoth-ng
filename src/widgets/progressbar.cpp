/**
 * @file src/widgets/progressbar.cpp
...
void progress_bar::draw_contents()
{
	...
	if(area.w >= 2 && area.h >= 2) {
		...
		SDL_FillRect(surf,&area,SDL_MapRGBA(surf->format,bcr,bcg,bcb));
		SDL_FillRect(surf,&inner_area,SDL_MapRGBA(surf->format,gcr,gcg,gcb));

		inner_area.w = (inner_area.w*progress_)/100;
		SDL_FillRect(surf,&inner_area,SDL_MapRGBA(surf->format,fcr,fcg,fcb));
		...
		SDL_FillRect(surf,&lightning,SDL_MapRGBA(surf->format,(fcr*3+255)/4,(fcg*3+255)/4,(fcb*3+255)/4));
		lightning.y = inner_area.y+inner_area.h-lightning.h;
		//remove 50% of color to simulate a shadow effect
		SDL_FillRect(surf,&lightning,SDL_MapRGBA(surf->format,fcr/2,fcg/2,fcb/2));
