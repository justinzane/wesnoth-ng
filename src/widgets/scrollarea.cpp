...
void scrollarea::update_location(SDL_Rect const &rect) {
    ...
    if ( !hidden()) scrollbar_.hide( !shown_scrollbar_);
...
void scrollarea::hide(bool value) {
    widget::hide(value);
    if (shown_scrollbar_) scrollbar_.hide(value);
}
...
void scrollarea::process_event() {
    ...
    if (grip_position == old_position_) return;
...
SDL_Rect scrollarea::inner_location() const {
    SDL_Rect r = location();
    if (shown_scrollbar_) r.w -= scrollbar_.width();
    return r;
}
...
void scrollarea::handle_event(const SDL_Event& event) {
    if (event == NULL) {
        throw std::invalid_argument("scrollarea::handle_event cannot handle a null event.");
    }

    if (mouse_locked() ||
        hidden() ||
        event.type != SDL_MOUSEWHEEL) { return; }

    SDL_MouseWheelEvent& wheel_evt = reinterpret_cast<SDL_MouseWheelEvent&>(event);
    if (point_in_rect(wheel_evt.x, wheel_evt.y, inner_location())) {
        if (wheel_evt.button == SDL_MOUSEWHEEL) {
            scrollbar_.scroll_down();
        } else if (wheel_evt.button == SDL_BUTTON_WHEELUP) {
