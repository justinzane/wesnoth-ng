...
menu::basic_sorter& menu::basic_sorter::set_position_sort(int column,
...
static int xp_to_advance(const std::string& s) {
    ...
    if (xp_frac.second == 0) xp_frac.second = 1000000;
...
bool menu::basic_sorter::less(int column, const item& row1, const item& row2) const {
    ...
    if (alpha_sort_.count(column) == 1) {
        std::string::const_iterator begin1 = item1.begin(), end1 = item1.end(), begin2 = item2
            .begin(), end2 = item2.end();
...
menu::menu(ui_window& video,
    ...
    help_string_( -1),
    selected_(0),
    click_selects_(click_selects),
    out_(false),
    previous_button_(true),
    show_result_(false),
...
void menu::fill_items(const std::vector<std::string>& items, bool strip_spaces) {
    for (std::vector<std::string>::const_iterator itor = items.begin(); itor != items.end();
        ++itor) {
...
class sort_func {
    public:
        sort_func(const menu::sorter& pred, int column) :
            pred_( &pred), column_(column) {
        }
...
void menu::do_sort() {
    ...
    if (sortreversed_) std::reverse(items_.begin(), items_.end());
...
void menu::create_help_strings() {
    for (std::vector<item>::iterator i = items_.begin(); i != items_.end(); ++i) {
        i->help.clear();
        for (std::vector<std::string>::iterator j = i->fields.begin(); j != i->fields.end();
            ++j) {
            if (std::find(j->begin(), j->end(), static_cast<char>(HELP_STRING_SEPARATOR)) == j
                ->end()) {
                i->help.push_back("");
            } else {
                const std::vector<std::string>& items = utils::split( *j, HELP_STRING_SEPARATOR,
                                                                     0);
                if (items.size() >= 2) {
                    *j = items.front();
...
void menu::update_size() {
    ...
    for (size_t i = get_position(), i_end = std::min(items_.size(), i + max_items_onscreen());
        i < i_end; ++i)
        h += get_item_rect(i).h;
    h = std::max(h, height());
    ...
    if (items_.size() > max_items_onscreen()) w += scrollbar_width();
    w = std::max(w, width());
...
void menu::change_item(int pos1, int pos2, const std::string& str) {
    if (pos1 < 0 || pos1 >= int(item_pos_.size()) || pos2 < 0
        || pos2 >= int(items_[item_pos_[pos1]].fields.size())) {
        return;
    }
...
void menu::erase_item(size_t index) {
    ...
    if (selected_ >= nb_items) selected_ = nb_items - 1;
...
size_t menu::max_items_onscreen() const {
    ...
    if (sum > max_height && n > 1) --n;

    return max_items_ = n;
}

void menu::adjust_viewport_to_selection() {
    if (click_selects_) return;
    adjust_position(selected_);
}

void menu::set_selection_pos(size_t new_selected,
                             bool silent,
                             SELECTION_MOVE_VIEWPORT move_viewport) {
    if (new_selected >= items_.size()) return;
...
void menu::key_press(SDL_Keycode key) {
    ...
    if (num_selects_ && key >= SDLK_1 && key <= SDLK_9) set_selection_pos(key - SDLK_1);
}
...
void menu::handle_event(const SDL_Event& event) {
    ...
    if (height() == 0 || hidden()) return;

    if (event.type == SDL_KEYDOWN) {
        // Only pass key events if we have the focus
        if (focus( &event)) key_press(event.key.keysym.sym);
...
SDL_Rect menu::style::item_size(const std::string& item) const {
    ...
    for (std::vector<std::string>::const_iterator it = img_text_items.begin();
        ...
        if ( !str.empty() && str[0] == IMAGE_PREFIX) {
            ...
            const SDL_Rect font_size = font::draw_text(NULL, area, get_font_size(),
...
void menu::style::draw_row_bg(menu& menu_ref,
    ...
    draw_solid_tinted_rectangle(rect.x, rect.y, rect.w, rect.h, (rgb & 0xff0000) >> 16,
                                (rgb & 0xff00) >> 8, rgb & 0xff, alpha,
                                menu_ref.video().getSurface());
}

void menu::style::draw_row(menu& menu_ref,
                           const size_t row_index,
                           const SDL_Rect& rect,
                           ROW_TYPE type) {
...
void menu::column_widths_item(const std::vector<std::string>& row,
    ...
    for (size_t col = 0; col != row.size(); ++col) {
        ...
        size_t text_trailing_space =
            (item_ends_with_image(row[col])) ? 0 : style_->get_cell_padding();
...
void menu::draw_row(const size_t row_index, const SDL_Rect& rect, ROW_TYPE type) {
    ...
    const std::vector<std::string>& row =
        (type == HEADING_ROW) ? heading_ : items_[row_index].fields;
    ...
    int xpos = rect.x;
    if (lang_rtl) xpos += rect.w;
    for (size_t i = 0; i != row.size(); ++i) {

        if (lang_rtl) xpos -= widths[i];
        if (type == HEADING_ROW && highlight_heading_ == int(i)) {
            draw_solid_tinted_rectangle(xpos, rect.y, widths[i], rect.h, 255, 255, 255, 0.3,
                                        video().getSurface());
        }
        ...
        for (std::vector<std::string>::const_iterator it = img_text_items.begin();
            ...
            if ( !str.empty() && str[0] == IMAGE_PREFIX) {
                ...
                const int remaining_width =
                    max_width_ < 0 ?
                        area.w :
                        std::min<int>(max_width_,
                                      ( (lang_rtl) ? xpos - rect.x : rect.x + rect.w - xpos));
                if (img != NULL && img->w <= remaining_width && rect.y + img->h < area.h) {
                ...
                font::draw_text( &video(), column, style_->get_font_size(), font::NORMAL_COLOR,
                                to_show, xpos, y);

                if (type == HEADING_ROW && sortby_ == int(i)) {
                    const surface sort_img = image::get_image(
                        sortreversed_ ? "buttons/sliders/slider_arrow_blue.png" :
...
void menu::draw() {
    ...
    clip_rect_setter clipping_rect = clip_rect_setter(video().getSurface(), clip_rect(),
                                                      clip_rect() != NULL);
...
int menu::hit(int x, int y) const {
    ...
    if (x >= loc.x && x < loc.x + loc.w && y >= loc.y && y < loc.y + loc.h) {
        for (size_t i = 0; i != items_.size(); ++i) {
            const SDL_Rect& rect = get_item_rect(i);
            if (y >= rect.y && y < rect.y + rect.h) return i;
        }
    }
...
SDL_Rect menu::get_item_rect_internal(size_t item) const {
    ...
    if (item < first_item_on_screen || item >= first_item_on_screen + max_items_onscreen()) {
        return empty_rect;
    }

    const std::map<int, SDL_Rect>::const_iterator i = itemRects_.find(item);
    if (i != itemRects_.end()) return i->second;
    ...
    //been initialized
    if (loc.x > 0 && loc.y > 0) itemRects_.insert(std::pair<int, SDL_Rect>(item, res));
...
size_t menu::get_item_height(int) const {
    if (item_height_ != -1) return size_t(item_height_);
