/**
 * @file src/widgets/combo_drag.cpp
...
combo_drag::combo_drag(display& disp,
                       ...
                       const drop_group_mgr_ptr group) :
    combo(disp, items),
...
void combo_drag::handle_move(const SDL_MouseMotionEvent& event) {
    ...
    if (drag_ == PRESSED_MOVE && std::sqrt(
        static_cast<float>(diff_x * diff_x + diff_y * diff_y))
                                 > MIN_DRAG_DISTANCE) {
        return;
    }
    drag_ = MOVED;
    ...
    if (clip_rect()) {
        ...
        if (loc.x < clip->x) loc.x = clip->x;
        if (loc.x + loc.w > clip->x + clip->w) loc.x = clip->x + clip->w - loc.w;
        if (loc.y < clip->y) loc.y = clip->y;
        if (loc.y + loc.h > clip->y + clip->h) loc.y = clip->y + clip->h - loc.h;
    }
...
void combo_drag::mouse_motion(const SDL_MouseMotionEvent& event) {
    if (drag_ == PRESSED || drag_ == MOVED || drag_ == PRESSED_MOVE) {
        handle_move(event);
    } else {
...
void combo_drag::mouse_up(const SDL_MouseButtonEvent& event) {
    if ( (drag_ == PRESSED || drag_ == PRESSED_MOVE || drag_ == MOVED) && event.button
        == SDL_BUTTON_LEFT) {
        if (drag_ == PRESSED || drag_ == PRESSED_MOVE) {
            free_mouse_lock();
            drag_ = DROP_DOWN;
        } else if (drag_ == MOVED) {
            free_mouse_lock();
