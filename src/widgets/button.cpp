...
button::button(CVideo& video,
               const std::string& label,
               button::button_type_t type,
               std::string button_image_name,
               button_space_consumption_t spacing,
    ...
    label_(label),
    spacing_(spacing),
    button_overlay_image_name_(overlay_image) {
...
void button::load_images() {
    ...
    if ( !button_overlay_image_name_.empty()) {
        overlayImage_.assign(
            image::get_image(button_overlay_image_name_ + size_postfix + ".png"));
        overlayPressedImage_.assign(
            image::get_image(button_overlay_image_name_ + size_postfix + "-pressed.png"));

        if (file_exists(
            game_config::path + "/images/" + button_overlay_image_name_ + size_postfix
            + "-active.png"))
            overlayActiveImage_.assign(
                image::get_image(button_overlay_image_name_ + size_postfix + "-active.png"));

        if (file_exists(
            game_config::path + "/images/" + button_overlay_image_name_ + size_postfix
            + "-disabled.png"))
            overlayDisabledImage_.assign(
                image::get_image(button_overlay_image_name_ + size_postfix + "-disabled.png"));
        if (overlayDisabledImage_.null())
            overlayDisabledImage_ = image::get_image(
                button_overlay_image_name_ + size_postfix + ".png~GS()");

        if (file_exists(
            game_config::path + "/images/" + button_overlay_image_name_ + size_postfix
            + "-disabled-pressed.png"))
            overlayPressedDisabledImage_.assign(
                image::get_image(
                    button_overlay_image_name_ + size_postfix + "-disabled-pressed.png"));
        if (overlayPressedDisabledImage_.null()) overlayPressedDisabledImage_ =
            image::get_image(button_overlay_image_name_ + size_postfix + "-pressed.png~GS()");
    } else {
    ...
    if (pressed_image.null()) pressed_image.assign(button_image);

    if (active_image.null()) active_image.assign(button_image);

    if (type_ == TYPE_CHECK || type_ == TYPE_RADIO) {
        touched_image.assign(image::get_image(button_image_name_ + "-touched.png"));
        if (touched_image.null()) touched_image.assign(pressed_image);

        pressed_active_image.assign(
            image::get_image(button_image_name_ + "-active-pressed.png"));
        if (pressed_active_image.null()) pressed_active_image.assign(pressed_image);

        if (file_exists(
            game_config::path + "/images/" + button_image_name_ + size_postfix
            + "-disabled-pressed.png"))
            pressed_disabled_image.assign(
                image::get_image(button_image_name_ + "-disabled-pressed.png"));
    ...
    if (type_ == TYPE_PRESS || type_ == TYPE_TURBO) {
        ...
        if (type_ == TYPE_CHECK || type_ == TYPE_RADIO) {
            pressedDisabledImage_.assign(
                scale_surface(pressed_disabled_image, button_image->w, button_image->h));
            pressedActiveImage_.assign(
                scale_surface(pressed_active_image, button_image->w, button_image->h));
            touchedImage_.assign(
                scale_surface(touched_image, button_image->w, button_image->h));
        }
    }
...
void button::calculate_size() {
    ...
    if ( !change_size) {
        unsigned w = loc.w
            - (type_ == TYPE_PRESS || type_ == TYPE_TURBO ?
                horizontal_padding : checkbox_horizontal_padding + base_width_);
        if (type_ != TYPE_IMAGE) {
            int fs = font_size;
            int style = TTF_STYLE_NORMAL;
            std::string::const_iterator i_beg = label_.begin(), i_end = label_.end(), i =
                font::parse_markup(i_beg, i_end, &fs, nullptr, &style);
            if (i != i_end) {
    ...
    if (type_ != TYPE_IMAGE) {
        textRect_ = font::draw_text(nullptr, screen_area(), font_size, font::BUTTON_COLOR,
                                    label_, 0, 0);
    }

    if ( !change_size) return;
...
void button::set_check(bool check) {
    if (type_ != TYPE_CHECK && type_ != TYPE_RADIO) return;
    ...
    if ( !overlayImage_.null()) {
        ...
        if ( !overlayPressedImage_.null()) {
            switch (state_) {
                ...
                case TOUCHED_NORMAL:
                case TOUCHED_PRESSED:
                    noverlay = make_neutral_surface(
                        enabled() ? overlayPressedImage_ : overlayPressedDisabledImage_);
                    break;
                default:
                    break;
            }
...
void button::set_label(const std::string& val) {
    ...
    if (std::find(label_.begin(), label_.end(), COLUMN_SEPARATOR) != label_.end()) {
        ...
        const std::vector<std::string>::const_iterator i = std::find_if(items.begin(),
                                                                        items.end(), not_image);
        if (i != items.end()) {
            label_ = *i;
        }
...
void button::mouse_motion(SDL_MouseMotionEvent const &event) {
    if (hit(event.x, event.y)) {
        ...
        if (state_ == NORMAL)
            state_ = ACTIVE;
        else if (state_ == PRESSED && (type_ == TYPE_CHECK || type_ == TYPE_RADIO)) state_ =
            PRESSED_ACTIVE;
    } else {
        ...
        } else if ( (type_ != TYPE_IMAGE) || state_ != PRESSED) state_ = NORMAL;
    }
}
    ...
    if (hit(event.x, event.y) && event.button == SDL_BUTTON_LEFT) {
        ...
        switch (type_) {
            ...
            case TYPE_CHECK:
                ...
                else if (state_ == ACTIVE) state_ = TOUCHED_NORMAL;
                break;
            case TYPE_TURBO:
...
void button::mouse_up(SDL_MouseButtonEvent const &event) {
    if ( ! (hit(event.x, event.y) && event.button == SDL_BUTTON_LEFT)) return;
...
void button::handle_event(const SDL_Event& event) {
    ...
    button_state_t start_state = state_;
    ...
    if (start_state != state_) set_dirty(true);
}
