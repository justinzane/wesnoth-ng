...
enum server_type {
    ABORT_SERVER, WESNOTHD_SERVER, SIMPLE_SERVER
};
        ...
        if(data.child("mustlogin")) {
            ...
            for(;;) {
                ...
                do {
                    ...
                    /* If the server asks for a password, provide one if we can or request a
                     * password reminder. Otherwise or if the user pressed 'cancel' in the
                     * confirmation dialog above go directly to the username/password dialog */
                    if((is_pw_request || !password_reminder.empty()) && !fall_through) {
                        if(is_pw_request) {
                            if ((*error)["phpbb_encryption"].to_bool()) {
                                ...
                                /** @todo FIXME with library password hashing. 2013-11-08 */
                                sp["password"] = util::get_hash(password);
...
static void enter_wait_mode(game_display& disp, const config& game_config, game_state& state,
bool observe) {
...
static void enter_create_mode(game_display& disp, const config& game_config, game_state& state,
bool local_players_only = false);

static bool enter_connect_mode(game_display& disp,
...
static bool enter_configure_mode(game_display& disp,
                                 const config& game_config,
                                 game_state& state,
                                 const mp_game_settings& params,
                                 bool local_players_only = false);
...
static void enter_lobby_mode(game_display& disp, const config& game_config, game_state& state) {
...
void start_local_game(game_display& disp, const config& game_config, game_state& state) {
...
void start_local_game_commandline(game_display& disp,
                                  const config& game_config,
                                  game_state& state,
...
void start_client(game_display& disp,
                  const config& game_config,
                  game_state& state,
...
mp::ui::result goto_mp_connect(game_display& disp,
    ...
    {
        mp::connect ui(disp, game_name, game_config, gamechat, gamelist, engine);
        run_lobby_loop(disp, ui);
...
mp::ui::result goto_mp_wait(game_state& state, game_display& disp, const config& game_config,
bool observe) {
    mp::ui::result res;
