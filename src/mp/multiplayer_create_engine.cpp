/**
 * @file src/mp/multiplayer_create_engine.cpp
 ...
 */
#include "mp/multiplayer_create_engine.hpp"
#include "mp/multiplayer_create_engine.hpp"
...
scenario::scenario(const config& data) :
    level(data), map_(), num_players_(0) {
}
...
SDL_Surface* scenario::create_image_surface(const SDL_Rect* image_rect) {
    SDL_Surface* minimap {nullptr};

    if (map_.get() != nullptr) {
        minimap = image::getMinimap(image_rect.w, image_rect.h, *map_, 0);
    }
...
void scenario::set_sides() {
    if (map_.get() != nullptr) {
        ...
        num_players_ = 0;
        foreach_ng(const config &scenario, data_.child_range("side")) {
...
random_map::random_map(const config& generator_data) :
    scenario(config()), generator_data_(generator_data) {
}
...
campaign::campaign(const config& data) :
    level(data), id_(data["id"]), image_label_(), min_players_(2), max_players_(2) {
}
...
surface* campaign::create_image_surface(const SDL_Rect* image_rect) {
    SDL_Surface temp_image(image::get_image(image::locator(image_label_)));

    surface* campaign_image = new surface(
        scale_surface(temp_image, image_rect.w, image_rect.h));
...
std::vector<std::string> create_engine::levels_menu_item_names() const {
    ...
    foreach_ng(level_ptr level, get_levels_by_type(current_level_type_)) {
...
std::vector<std::string> create_engine::extras_menu_item_names(const MP_EXTRA extra_type) const {
    ...
    foreach_ng(extras_metadata_ptr extra,
...
const create_engine::extras_metadata& create_engine::current_extra(const MP_EXTRA extra_type) const {
    const size_t index = (extra_type == ERA) ? current_era_index_ : current_mod_index_;
...
void create_engine::set_current_level(const size_t index) {
    ...
    if (current_level_type_ == level::RANDOM_MAP) {
        ...
        generator_.assign(
            create_map_generator(current_random_map->generator_data()["map_generation"],
...
int create_engine::find_level_by_id(const std::string& id) const {
    int i = 0;
    foreach_ng(user_map_ptr user_map, user_maps_) {
    if (user_map->id() == id) {
        return i;
    }
    i++;
    ...
    i = 0;
    foreach_ng(random_map_ptr random_map, random_maps_) {
    if (random_map->id() == id) {
        return i;
    }
    i++;
    ...
    i = 0;
    foreach_ng(scenario_ptr scenario, scenarios_) {
    if (scenario->id() == id) {
        return i;
    }
    i++;
    ...
    i = 0;
    foreach_ng(campaign_ptr campaign, campaigns_) {
    if (campaign->id() == id) {
        return i;
    }
    i++;
    ...
    i = 0;
    foreach_ng(campaign_ptr sp_campaign, sp_campaigns_) {
...
std::vector<create_engine::level_ptr> create_engine::get_levels_by_type(level::TYPE type) const {
    ...
    switch (type) {
        case level::SCENARIO:
            foreach_ng(level_ptr level, scenarios_) {
            levels.push_back(level);
        }
        break;
        case level::USER_MAP:
        foreach_ng(level_ptr level, user_maps_) {
            levels.push_back(level);
        }
        break;
        case level::RANDOM_MAP:
        foreach_ng(level_ptr level, random_maps_) {
            levels.push_back(level);
        }
        break;
        case level::CAMPAIGN:
        foreach_ng(level_ptr level, campaigns_) {
            levels.push_back(level);
        }
        break;
        case level::SP_CAMPAIGN:
        foreach_ng(level_ptr level, sp_campaigns_) {
            levels.push_back(level);
        }
        break;
    }  // end switch
