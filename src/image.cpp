...
class cache_type {
    public:
        ...
        cache_item<T> &get_element(int index) {
            if (static_cast<unsigned>(index) >= content_.size()) content_.resize(index + 1);
...
void locator::add_to_cache(cache_type<T> &cache, const T &data) const {
    if (index_ >= 0) cache.get_element(index_) = cache_item<T>(data);
}
...
image::image_cache images_, scaled_to_zoom_, hexed_images_, scaled_to_hex_images_,
    tod_colored_images_, brightened_images_;
...
locator::locator(const std::string &filename,
    ...
    index_( -1), val_(filename, loc, center_x, center_y, modifications) {
    init_index();
}
...
bool locator::value::operator<(const value& a) const {
    if (type_ != a.type_) {
        ...
        if (filename_ != a.filename_) return filename_ < a.filename_;
        if (loc_ != a.loc_) return loc_ < a.loc_;
        if (center_x_ != a.center_x_) return center_x_ < a.center_x_;
        if (center_y_ != a.center_y_) return center_y_ < a.center_y_;
...
static bool localized_file_uptodate(const std::string& loc_file) {
    if (fuzzy_localized_files.empty()) {
        ...
        foreach_ng(const std::string &line, lines) {
...
static std::string get_localized_path(const std::string& file, const std::string& suff = "") {
    ...
    std::vector<std::string> langs = utils::split(
        _("language code for localized resources^en_US"));
    ...
    foreach_ng(const std::string &lang, langs) {
...
static SDL_Surface apply_light(SDL_Surface surf, const light_string& ls) {
    ...
    if (i != lightmaps_.end()) {
        ...
        static const std::string lm_img[7] = { p + "-n.png", p + "-ne.png", p + "-se.png", p
            + "-s.png",
                                               p + "-sw.png", p + "-nw.png", p + ".png" };
...
SDL_Surface locator::load_image_sub_file() const {
    ...
    if (val_.loc_.valid()) {
        SDL_Rect srcrect = create_rect(
            ...
            tile_size * val_.loc_.y + (tile_size / 2) * (val_.loc_.x % 2), tile_size,
            tile_size);
...
void set_pixel_format(SDL_PixelFormat* format) {
    ...
    if (format->BitsPerPixel == 8 || f.BitsPerPixel != l.BitsPerPixel
        || f.BytesPerPixel != l.BytesPerPixel || f.Rmask != l.Rmask || f.Gmask != l.Gmask
        || f.Bmask != l.Bmask
...
static SDL_Surface get_scaled_to_zoom(const locator& i_locator) {
    ...
    if ( !res.null()) {
        return scale_surface(res, ( (res.get()->w * zoom) / tile_size),
                             ( (res.get()->h * zoom) / tile_size));
    } else {
        return surface(nullptr);
    }
}
...
static TYPE simplify_type(const image::locator& i_locator, TYPE type) {
    switch (type) {
        case SCALED_TO_ZOOM:
            if (zoom == tile_size) type = UNSCALED;
            break;
        case BRIGHTENED:
            if (ftofxp(game_config::hex_brightening) == ftofxp(1.0)) type = TOD_COLORED;
            break;
        default:
            break;
    }
    ...
    if (type == TOD_COLORED) {
        if (red_adjust == 0 && green_adjust == 0 && blue_adjust == 0) type = SCALED_TO_HEX;
    }

    if (type == SCALED_TO_HEX) {
        if (zoom == tile_size) type = HEXED;
    }

    if (type == HEXED) {
        // check if the image is already hex-cut by the location system
        if (i_locator.get_loc().valid()) type = UNSCALED;
    }
...
SDL_Surface get_image(const image::locator& i_locator, TYPE type) {
    ...
    if (i_locator.is_void()) return res;
...
#pragma omp critical(image_cache)
    ...
    if (res) res = create_optimized_surface(res);
...
SDL_Surface get_lighted_image(const image::locator& i_locator,
    ...
    if (type == SCALED_TO_HEX) imap = &lit_scaled_images_;
...
bool exists(const image::locator& i_locator) {
    ...
    if (type != loc::FILE && type != loc::SUB_FILE) return false;

    // The insertion will fail if there is already an element in the cache
    std::pair<std::map<std::string, bool>::iterator, bool> it = image_existence_map.insert(
        std::make_pair(i_locator.get_filename(), false));
    bool &cache = it.first->second;
    if (it.second) cache =
        !get_binary_file_location("images", i_locator.get_filename()).empty();
    return cache;
}

static void precache_file_existence_internal(const std::string& dir,
                                             const std::string& subdir) {
    const std::string checked_dir = dir + "/" + subdir;
    if (precached_dirs.find(checked_dir) != precached_dirs.end()) return;
    precached_dirs.insert(checked_dir);

    std::vector<std::string> files_found;
    std::vector<std::string> dirs_found;
    get_files_in_dir(checked_dir, &files_found, &dirs_found, FILE_NAME_ONLY, NO_FILTER,
                     DONT_REORDER);
    ...
    for (std::vector<std::string>::const_iterator d = dirs_found.begin(); d != dirs_found.end();
        ++d) {
