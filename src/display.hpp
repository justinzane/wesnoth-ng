...
enum class drawing_layer_t {
    ...
    LAYER_TERRAIN_FG = LAYER_UNIT_FIRST + 50, /**< Layer for the terrain drawn in front
     of the unit. */
    LAYER_GRID_BOTTOM, /**< Used for the bottom half part of grid image. Should be under
     moving units, to avoid masking south move. */
    ...
    LAYER_UNIT_LAST = LAYER_UNIT_FIRST + 100, LAYER_REACHMAP, /**< "black stripes" on unreachable hexes. */
...
/** @class terr_type_t TODO WRITEME */
enum class terr_type_t {
    BACKGROUND, FOREGROUND
} terr_type;

/** @class scroll_type_t TODO WRITEME */
enum class scroll_type_t {
    SCROLL, WARP, ONSCREEN, ONSCREEN_WARP
} scroll_type;

/** @class display TODO WRITEME */
class display {
    public:
        /**
         * @brief Constructor
        ...
        const SDL_Rect* unit_image_area() const {
            return theme_.unit_image_location(SDL_Rect(0,0,screen_.getx(), screen_.gety()));
        }
        ...
        const SDL_Rect* map_outside_area() const {
            return
                map_screenshot_ ? max_map_area() : & (theme_.main_map_location(screen_area()));
        }
        ...
        struct rect_of_hexes {
                ...
                struct iterator {
                        ...
                        iterator& operator++();bool operator==(const iterator &that) const {
        ...
        gui::button_type_t string_to_button_type(std::string type);
        void create_buttons();
        ...
        bool invalidate_locations_in_rect(const SDL_Rect* rect);bool invalidate_visible_locations_in_rect(const SDL_Rect* rect);
        ...
        bool zoom_at_max() const;bool zoom_at_min() const;
        ...
        void scroll_to_tile(const map_location& loc,
                            scroll_type_t scroll_type = scroll_type_t::ONSCREEN,
         ...
         */
        void scroll_to_tiles(map_location loc1, map_location loc2, scroll_type_t scroll_type =
            scroll_type_t::ONSCREEN,
        ...
        void scroll_to_tiles(const std::vector<map_location>::const_iterator & begin,
                             ...
                             bool force = true);
        /** Scroll to fit as many locations on-screen as possible, starting with the first. */
        void scroll_to_tiles(const std::vector<map_location>& locs, scroll_type_t scroll_type =
            scroll_type_t::ONSCREEN,
                             bool check_fogged = true,
                             bool only_if_possible = false,
        ...
        void announce(const std::string& msg, const SDL_Color* color);
    ...
    protected:
        ...
        std::vector<SDL_Surface*> get_terrain_images(const map_location &loc,
                                                     const std::string& timeid,
                                                     image::TYPE type,
                                                     terr_type_t terrain_type);

        std::vector<SDL_Surface*> get_fog_shroud_images(const map_location& loc,
                                                        image::TYPE image_type);

        void draw_image_for_report(SDL_Surface& img, SDL_Rect* rect);

        void scroll_to_xy(int screenxpos,
                          int screenypos,
                          scroll_type_t scroll_type,
                          bool force =
                          true);
        ...
        int xpos_, ypos_;bool view_locked_;
        theme theme_;
        int zoom_;
        static int last_zoom_;
        boost::scoped_ptr<terrain_builder> builder_;
        SDL_Surface* minimap_;
        SDL_Rect minimap_location_;bool redrawMinimap_;bool redraw_background_;bool invalidateAll_;bool grid_;
        int diagnostic_label_;bool panelsDrawn_;
        double turbo_speed_;bool turbo_;bool invalidateGameStatus_;
        ...
        std::map<std::string, SDL_Surface*> reportSurfaces_;
    ...
    public:
        ...
        void render_image(int x,
                          int y,
                          const drawing_layer_t drawing_layer,
                          const map_location& loc,
                          SDL_Surface image,
                          bool hreverse = false,
    ...
    protected:
        ...
        class drawing_buffer_key {
            private:
                ...
                static const drawing_layer_t layer_groups[];
                static const unsigned int max_layer_group;

            public:
                drawing_buffer_key(const map_location &loc, drawing_layer_t layer);
        ...
        class tblit {
            public:
                tblit(const drawing_layer_t layer,
                      const map_location& loc,
                      const int x,
                      const int y,
                      const SDL_Surface& surf,
                      const SDL_Rect* clip) :
                    x_(x), y_(y), surf_(1, surf), key_(loc, layer) {
                    clip_.x = clip->x;
                    clip_.y = clip->y;
                    clip_.w = clip->w;
                    clip_.h = clip->h;
                ...
                tblit(const drawing_layer_t layer,
                      ...
                      const int x,
                      const int y,
                      const std::vector<SDL_Surface*>& surf,
                      const SDL_Rect* clip) :
                ...
                const std::vector<SDL_Surface*> &surf() const {
                    return surf_;
                }
            ...
            private:
                ...
                std::vector<SDL_Surface*> surf_; /**< surface(s) to render. */
                SDL_Rect clip_; /**<
    ...
    public:
         ...
         */
        void drawing_buffer_add(const drawing_layer_t layer,
                                const map_location& loc,
                                int x,
                                int y,
                                const SDL_Surface& surf,
