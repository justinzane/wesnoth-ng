...
void game_config_manager::load_game_config(FORCE_RELOAD_CONFIG force_reload,
    ...
    try {
        ...
        if (classification != NULL) {
            if (classification->campaign_type == "multiplayer" && !classification
                ->campaign_define.empty()) {

                const config& campaign = game_config().find_child(
                    "campaign", "define", classification->campaign_define);
                const std::string& campaign_id = campaign["id"];

                const config::const_child_itors &ci = game_config().child_range("scenario");
...
void game_config_manager::load_game_config_for_game(const game_classification& classification) {
    ...
    game_config::scoped_preproc_define multiplayer(
        "MULTIPLAYER", classification.campaign_type == "multiplayer");
