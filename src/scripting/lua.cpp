...
static void luaW_pushvconfig(lua_State *L, vconfig const &cfg) {
    ...
    lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &vconfigKey)));
...
static void luaW_pushtstring(lua_State *L, t_string const &v) {
    ...
    lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &tstringKey)));
...
static t_string luaW_checktstring(lua_State *L, int index) {
    t_string result;
    if ( !luaW_totstring(L, index, result)) luaL_typerror(L, index, "translatable string");
    return result;
}
...
static void luaW_filltable(lua_State *L, config const &cfg) {
    if ( !lua_checkstack(L, LUA_MINSTACK)) return;
...
bool luaW_toconfig(lua_State *L, int index, config &cfg, int tstring_meta) {
    if ( !lua_checkstack(L, LUA_MINSTACK)) return false;

    // Get the absolute index of the table.
    int initial_top = lua_gettop(L);
    if ( -initial_top <= index && index <= -1) index = initial_top + index + 1;

    switch (lua_type(L, index)) {
        case LUA_TTABLE:
            break;
        case LUA_TUSERDATA: {
            if ( !luaW_hasmetatable(L, index, vconfigKey)) return false;
...
static config luaW_checkconfig(lua_State *L, int index) {
    config result;
    if ( !luaW_toconfig(L, index, result)) luaL_typerror(L, index, "WML table");
    return result;
}
...
bool luaW_pcall(lua_State *L, int nArgs, int nRets, bool allow_wml_error) {
    ...
    lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &executeKey)));
...
static int intf_textdomain(lua_State *L) {
    ...
    memcpy(p, m, l + 1);
    lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &gettextKey)));
...
static int impl_unit_get(lua_State *L) {
    ...
    if (strcmp(m, "status") == 0) {
        ...
        lua_pushvalue(L, 1);
        lua_rawseti(L, -2, 1);
        lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &ustatusKey)));
        lua_rawget(L, LUA_REGISTRYINDEX);
        lua_setmetatable(L, -2);
        return 1;
    }
    if (strcmp(m, "variables") == 0) {
        ...
        lua_pushvalue(L, 1);
        lua_rawseti(L, -2, 1);
        lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &unitvarKey)));
        lua_rawget(L, LUA_REGISTRYINDEX);
        lua_setmetatable(L, -2);
        return 1;
    }
...
static int impl_unit_set(lua_State *L) {
    ...
    // Find the corresponding attribute.
    modify_int_attrib_check_range("side", u.set_side(value), 1,
...
static int impl_unit_status_get(lua_State *L) {
    if ( !lua_istable(L, 1)) return luaL_typerror(L, 1, "unit status");
    lua_rawgeti(L, 1, 1);
...
static int impl_unit_variables_get(lua_State *L) {
    if ( !lua_istable(L, 1)) return luaL_typerror(L, 1, "unit variables");
    lua_rawgeti(L, 1, 1);
    unit const *u = luaW_tounit(L, -1);
    if ( !u) return luaL_argerror(L, 1, "unknown unit");
    char const *m = luaL_checkstring(L, 2);
...
static int intf_get_unit(lua_State *L) {
    ...
    lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &getunitKey)));
    lua_rawget(L, LUA_REGISTRYINDEX);
    lua_setmetatable(L, -2);
    return 1;
}
...
static int intf_get_units(lua_State *L) {
    ...
    int i = 1;
    unit_map &units = *resources::units;
    for (unit_map::const_unit_iterator ui = units.begin(), ui_end = units.end(); ui != ui_end;
        ++ui) {
        if ( !filter.null() && !ui->matches_filter(filter, ui->get_location())) continue;
        new (lua_newuserdata(L, sizeof(lua_unit))) lua_unit(ui->underlying_id());
        lua_pushvalue(L, 1);
...
static int intf_match_unit(lua_State *L) {
    if ( !luaW_hasmetatable(L, 1, getunitKey)) return luaL_typerror(L, 1, "unit");
    ...
    if (int side = lu->on_recall_list()) {
        ...
        scoped_recall_unit auto_store("this_unit", t.save_id(), u - &t.recall_list()[0]);
...
static int intf_get_recall_units(lua_State *L) {
    ...
    BOOST_FOREACH(team &t, *resources::teams) {
        BOOST_FOREACH(unit &u, t.recall_list()) {
            if ( !filter.null()) {
                scoped_recall_unit auto_store("this_unit", t.save_id(),
                                              &u - &t.recall_list()[0]);
                if ( !u.matches_filter(filter, map_location())) continue;
            }
...
static int intf_fire_event(lua_State *L) {
    ...
    bool b = game_events::fire((std::string)m,
                               (game_events::entity_location)l1,
                               (game_events::entity_location)l2,
                               data);
    lua_pushboolean(L, b);
    return 1;
}
...
static int intf_get_variable(lua_State *L) {
    ...
    if (v.is_valid) {
        ...
        if (w.is_valid) {
            lua_newtable(L);
            if (lua_toboolean(L, 2)) luaW_filltable(L, w.as_container());
            return 1;
        }
    }
    return 0;
...
static int intf_set_variable(lua_State *L) {
    ...
    switch (lua_type(L, 2)) {
        ...
        case LUA_TTABLE: {
            ...
            cfg.clear();
            if (luaW_toconfig(L, 2, cfg)) break;
            // no break
        }
        default:
...
static int intf_dofile(lua_State *L) {
    char const *m = luaL_checkstring(L, 1);
    std::string p = get_wml_location(m);
    if (p.empty()) return luaL_argerror(L, 1, "file not found");

    lua_settop(L, 0);
    if (luaL_loadfile(L, p.c_str())) return lua_error(L);
...
static int intf_get_terrain(lua_State *L) {
    ...
    t_translation::t_terrain const &t = resources::game_map->get_terrain(
        map_location(x - 1, y - 1));
...
static int intf_get_time_of_day(lua_State *L) {
    ...
    if (lua_istable(L, arg)) {
        ...
        if ( !resources::game_map->on_board(loc))
            return luaL_argerror(L, arg, "coordinates are not on board");
        lua_pop(L, 2);
...
static int intf_get_village_owner(lua_State *L) {
    ...
    if ( !resources::game_map->is_village(loc)) return 0;
...
static int intf_set_village_owner(lua_State *L) {
    ...
    if (new_side == old_side || new_side < 0 || new_side > static_cast<int>(teams.size())
        || (new_side && !resources::units->find_leader(new_side).valid())) return 0;
...
struct lua_calculator: pathfind::cost_calculator {
        ...
        lua_calculator(lua_State *L_, int i) :
            L(L_), index(i) {
        }
...
static int intf_find_path(lua_State *L) {
    ...
    if ( !resources::game_map->on_board(src)) return luaL_argerror(L, 1, "invalid location");
    ...
    if (lua_istable(L, arg)) {
        ...
        lua_rawget(L, arg);
        if ( !lua_isnil(L, -1)) stop_at = luaL_checknumber(L, -1);
        lua_pop(L, 1);
    ...
    if ( !calc) {
        ...
        if ( !ignore_teleport) {
            teleport_locations = pathfind::get_teleport_locations( *u, viewing_team, see_all,
                                                                  ignore_units);
        }
        calc = new pathfind::shortest_path_calculator( *u, viewing_team, teams, map,
                                                      ignore_units, false, see_all);
    }

    pathfind::plain_route res = pathfind::a_star_search(src, dst, stop_at, calc, map.w(),
                                                        map.h(), &teleport_locations);
    delete calc;
...
static int intf_find_reach(lua_State *L) {
    ...
    if (lua_isuserdata(L, arg)) {
        ...
        if ( !ui.valid()) return luaL_argerror(L, 1, "unit not found");
        u = & *ui;
        ++arg;
    }
    ...
    team &viewing_team = teams[ (viewing_side ? viewing_side : u->side()) - 1];
    pathfind::paths res( *u, ignore_units, !ignore_teleport, viewing_team, additional_turns,
...
static int intf_find_cost_map(lua_State *L) {
    ...
    pathfind::full_cost_map cost_map(ignore_units, !ignore_teleport, viewing_team, see_all,
                                     ignore_units);
...
static int intf_put_recall_unit(lua_State *L) {
    ...
    if (luaW_hasmetatable(L, 1, getunitKey)) {
        ...
        u = lu->get();
        if ( !u || lu->on_recall_list()) return luaL_argerror(L, 1, "unit not found");
    } else {
    ...
    if ( !side) side = u->side();
    team &t = ( *resources::teams)[side - 1];
    if ( !t.persistent()) return luaL_argerror(L, 2, "nonpersistent side");
    std::vector<unit> &rl = t.recall_list();
    ...
    rl.push_back( *u);
    if (lu) {
        if (lu->on_map()) resources::units->erase(u->get_location());
        lu->lua_unit::~lua_unit();
...
static int intf_find_vacant_tile(lua_State *L) {
    ...
    map_location res = find_vacant_tile(map_location(x, y), pathfind::VACANT_ANY, u);
...
static int intf_float_label(lua_State *L) {
    ...
    resources::screen->float_label(loc, text, font::LABEL_COLOR.r, font::LABEL_COLOR.g,
                                   font::LABEL_COLOR.b);
    return 0;
}
...
static int intf_simulate_combat(lua_State *L) {
    ...
    battle_context context( *resources::units, att->get_location(), def->get_location(), att_w,
                           def_w, 0.0, NULL, att);
...
static int intf_scroll_to_tile(lua_State *L) {
    ...
    resources::screen->scroll_to_tile(map_location(x, y),
                                      immediate ? game_display::WARP : game_display::SCROLL,
                                      check_fogged);
    return 0;
}
...
static int intf_compare_versions(lua_State* L) {
    ...
    if (vop == OP_INVALID)
        return luaL_argerror(
            L, 2, "unknown version comparison operator - allowed are ==, !=, <, <=, > and >=");
...
static int intf_select_hex(lua_State *L) {
    ...
    bool highlight = true;
    if ( !lua_isnoneornil(L, 3)) highlight = lua_toboolean(L, 3);
    const bool fire_event = lua_toboolean(L, 4);
    resources::controller->get_mouse_handler_base().select_hex(loc, false, highlight,
                                                               fire_event);
    if (highlight) resources::screen->highlight_hex(loc);
    return 0;
}
...
struct lua_synchronize: mp_sync::user_choice {
        ...
        virtual config query_user() const {
            ...
            if ( !lua_isnoneornil(L, 2)) {
                ...
                if ( ( *resources::teams)[side - 1].is_ai()) index = 2;
            }
            lua_settop(L, index);
            if (luaW_pcall(L, 0, 1, false)) luaW_toconfig(L, -1, cfg);
            return cfg;
        }
...
scoped_dialog::scoped_dialog(lua_State *l, gui2::twindow *w) :
    ...
    lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &dlgclbkKey)));
    lua_createtable(L, 1, 0);
    lua_pushvalue(L, -2);
    lua_rawget(L, LUA_REGISTRYINDEX);
    lua_rawseti(L, -2, 1);
...
static gui2::twidget *find_widget(lua_State *L, int i, bool readonly) {
    if ( !scoped_dialog::current) {
        ...
        error_call_destructors_1: luaL_argerror(L, i, "out of bounds");
        error_call_destructors_2: luaL_typerror(L, i, "string");
        error_call_destructors_3: luaL_argerror(L, i, "widget not found");
        return NULL;
    }
    ...
    for (; !lua_isnoneornil(L, i); ++i) {
...
#endif
        {
            ...
            if (v < 1) goto error_call_destructors_1;
            int n = l->get_item_count();
            if (v > n) {
                if (readonly) goto error_call_destructors_1;
...
static int intf_set_dialog_callback(lua_State *L) {
    ...
    if (gui2::tclickable_ *c = dynamic_cast<gui2::tclickable_ *>(w)) {
        ...
        c->connect_click_handler(boost::bind( &tdialog_callback_wrapper::forward, wrapper, w));
    } else if (gui2::tselectable_ *s = dynamic_cast<gui2::tselectable_ *>(w)) {
...
static int intf_set_dialog_canvas(lua_State *L) {
    ...
    if (i < 1 || unsigned(i) > cv.size()) return luaL_argerror(L, 1, "out of bounds");
...
static int intf_get_sides(lua_State* L) {
    ...
    lua_settop(L, 0);
    lua_pushlightuserdata(L, static_cast<void*>(const_cast<char *>( &getsideKey)));
    lua_rawget(L, LUA_REGISTRYINDEX);
...
static void push_component(lua_State *L, ai::component* c, const std::string &ct = "") {
    ...
    for (std::vector<std::string>::const_iterator t = c_types.begin(); t != c_types.end();
        t++) {
        ...
        for (std::vector<ai::component*>::const_iterator i = children.begin();
            i != children.end(); i++) {
...
static int intf_debug_ai(lua_State *L) {
    ...
    for (std::vector<ai::component*>::const_iterator i = engines.begin(); i != engines.end();
        i++) {
        if ( ( *i)->get_name() == "lua") {
...
config lua_report_generator::generate() {
    lua_State *L = mState;
    config cfg;
    if ( !luaW_getglobal(L, "wesnoth", "theme_items", name.c_str(), NULL)) return cfg;
...
LuaKernel::LuaKernel(const config &cfg) :
    ...
    static const luaL_Reg safe_libs[] = { { "", luaopen_base }, { "table", luaopen_table }, {
        "string", luaopen_string },
                                          { "math", luaopen_math }, { "debug", luaopen_debug },
                                          { "os", luaopen_os }, { NULL, NULL } };
    ...
    static luaL_Reg const callbacks[] = { { "add_known_unit", &intf_add_known_unit }, {
        "add_modification", &intf_add_modification },
                                          ...
                                          { "create_unit", &intf_create_unit }, { "debug",
                                                                                  &intf_debug },
                                          { "debug_ai", &intf_debug_ai },
                                          { "delay", &intf_delay }, { "dofile", &intf_dofile },
                                          ...
                                          { "get_unit", &intf_get_unit }, { "get_units",
                                                                            &intf_get_units },
                                          ...
                                          { "match_unit", &intf_match_unit }, { "message",
                                                                                &intf_message },
                                          ...
                                          { "view_locked", &intf_view_locked }, { NULL, NULL } };
    ...
    lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &gettypeKey)));
    lua_createtable(L, 0, 2);
    ...
    lua_pushlightuserdata(L, static_cast<void *>(const_cast<char *>( &getraceKey)));
    lua_createtable(L, 0, 2);
...
static char const *handled_file_tags[] = { "color_palette", "color_range", "display",
                                           "endlevel", "era", "event", "generator", "label",
                                           "lua", "map", "menu_item", "modification", "music",
                                           "options", "side", "sound_source", "story",
                                           "terrain_graphics", "time", "time_area", "tunnel",
...
void LuaKernel::load_game() {
    ...
    if ( !luaW_getglobal(L, "wesnoth", "game_events", "on_load", NULL)) return;
...
void LuaKernel::save_game(config &cfg) {
    ...
    if ( !luaW_getglobal(L, "wesnoth", "game_events", "on_save", NULL)) return;

    if ( !luaW_pcall(L, 0, 1, false)) return;
...
bool LuaKernel::run_event(game_events::queued_event const &ev) {
    ...
    if ( !luaW_getglobal(L, "wesnoth", "game_events", "on_event", NULL)) return false;
...
static int cfun_wml_action(lua_State *L) {
    game_events::wml_action::handler h =
        reinterpret_cast<game_events::wml_action::handler>(lua_touserdata(L,
                                                                          lua_upvalueindex(1)));
 ...
 */
bool LuaKernel::run_wml_action(std::string const &cmd,
                               vconfig const &cfg,
                               game_events::queued_event const &ev) {
    lua_State *L = mState;

    if ( !luaW_getglobal(L, "wesnoth", "wml_actions", cmd.c_str(), NULL)) return false;
...
ai::lua_ai_action_handler* LuaKernel::create_lua_ai_action_handler(char const *code,
