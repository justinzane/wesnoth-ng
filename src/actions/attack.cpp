...
battle_context_unit_stats::battle_context_unit_stats(const unit &u,
    ...
    if (weapon) {
        ...
        if (opp_weapon) opp_weapon->set_specials_context(opp_loc, u_loc, !attacking, weapon);
        slows = weapon->get_special_bool("slow");
        drains = !opp.get_state("undrainable") && weapon->get_special_bool("drains");
        petrifies = weapon->get_special_bool("petrifies");
        poisons = !opp.get_state("unpoisonable") && weapon->get_special_bool("poison")
        ...
        plagues = !opp.get_state("unplagueable") && !plague_specials.empty()
                  && strcmp(opp.undead_variation().c_str(), "null")
                  && !resources::game_map->is_village(opp_loc);

        if (plagues) {
            plague_type = ( *plague_specials.front().first)["type"].str();
            if (plague_type.empty()) plague_type = u.type().base_id();
        }

        // Compute chance to hit.
        chance_to_hit = opp.defense_modifier(resources::game_map->get_terrain(opp_loc))
            + weapon->accuracy()
        ...
        // Leadership bonus.
        int leader_bonus = 0;
        if (under_leadership(units, u_loc, &leader_bonus).valid()) damage_multiplier +=
            leader_bonus;
        // Resistance modifier.
        ...
        if (is_slowed) damage = slow_damage;
...
battle_context_unit_stats::battle_context_unit_stats(const unit_type* u_type,
    ...
    if (weapon) {
        ...
        slows = weapon->get_special_bool("slow");
        drains = !opp_type->musthave_status("undrainable")
            && weapon->get_special_bool("drains");
        petrifies = weapon->get_special_bool("petrifies");
        poisons = !opp_type->musthave_status("unpoisonable")
...
battle_context::battle_context(const unit_map& units,
    ...
    if (attacker_weapon == -1 && attacker.attacks().size() == 1
        && attacker.attacks()[0].attack_weight() > 0) attacker_weapon = 0;

    if (attacker_weapon == -1) {
        attacker_weapon = choose_attacker_weapon(attacker, defender, units, attacker_loc,
                                                 defender_loc, harm_weight, &defender_weapon,
                                                 prev_def);
    } else if (defender_weapon == -1) {
        defender_weapon = choose_defender_weapon(attacker, defender, attacker_weapon, units,
    ...
    if ( !attacker_stats_) {
        ...
        attacker_stats_ = new battle_context_unit_stats(attacker, attacker_loc, attacker_weapon,
                                                        ...
                                                        units);
        defender_stats_ = new battle_context_unit_stats(defender, defender_loc, defender_weapon,
                                                        false, attacker, attacker_loc, adef,
                                                        units);
    }
}
...
battle_context& battle_context::operator=(const battle_context &other) {
    if ( &other != this) {
        ...
        attacker_combatant_ =
            other.attacker_combatant_ ?
                new combatant( *other.attacker_combatant_, *attacker_stats_) : NULL;
        defender_combatant_ =
            other.defender_combatant_ ?
                new combatant( *other.defender_combatant_, *defender_stats_) : NULL;
    }
    return *this;
}
...
bool battle_context::better_attack(class battle_context &that, double harm_weight) {
    return better_combat(get_attacker_combatant(), get_defender_combatant(),
                         that.get_attacker_combatant(), that.get_defender_combatant(),
                         harm_weight);
}
    ...
    for (i = 0; i < choices.size(); ++i) {
        ...
        if ( !attacker_combatant_) {
            ...
            attacker_stats_ = new battle_context_unit_stats(attacker, attacker_loc, choices[i],
                                                            ...
                                                            units);
            defender_stats_ = new battle_context_unit_stats(defender, defender_loc, def_weapon,
                                                            false, attacker, attacker_loc, &att,
                                                            units);
            attacker_combatant_ = new combatant( *attacker_stats_);
            defender_combatant_ = new combatant( *defender_stats_, prev_def);
            attacker_combatant_->fight( *defender_combatant_);
...
int battle_context::choose_defender_weapon(const unit &attacker,
    ...
    int min_rating = 0;
    {
        ...
        for (i = 0; i < choices.size(); ++i) {
            ...
            if (def.defense_weight() >= max_weight) {
                const battle_context_unit_stats def_stats(defender, defender_loc, choices[i],
                                                          false, attacker, attacker_loc, &att,
                                                          units);
    ...
    for (i = 0; i < choices.size(); ++i) {
        const attack_type &def = defender.attacks()[choices[i]];
        battle_context_unit_stats *att_stats = new battle_context_unit_stats(attacker,
                                                                             attacker_loc,
                                                                             attacker_weapon,
                                                                             true, defender,
                                                                             defender_loc, &def,
                                                                             units);
        battle_context_unit_stats *def_stats = new battle_context_unit_stats(defender,
                                                                             defender_loc,
                                                                             choices[i], false,
                                                                             attacker,
                                                                             attacker_loc, &att,
        ...
        if (simple_rating >= min_rating && ( !attacker_combatant_
            || better_combat( *def_comb, *att_comb, *defender_combatant_, *attacker_combatant_,
                             1.0))) {
            delete attacker_combatant_;
            delete defender_combatant_;
            delete attacker_stats_;
...
namespace {
void refresh_weapon_index(int& weap_index,
                          std::string const& weap_id,
    ...
    if (weap_index >= 0 && weap_index < static_cast<int>(attacks.size())
...
class attack {
    ...
    private:
        ...
        struct unit_info {
                ...
                int weapon_;
                unit_map &units_;
                std::string id_; /**< unit.underlying_id() */
                std::string weap_id_;
                int orig_attacks_;
...
void attack::refresh_bc() {
    ...
    if ( !a_.valid() || !d_.valid()) {
        ...
        const_cast<battle_context_unit_stats*>(a_stats_)->weapon =
            a_.valid() && a_.weapon_ >= 0 ? &a_.get_unit().attacks()[a_.weapon_] : NULL;

        const_cast<battle_context_unit_stats*>(d_stats_)->weapon =
            d_.valid() && d_.weapon_ >= 0 ? &d_.get_unit().attacks()[d_.weapon_] : NULL;
...
bool attack::perform_hit(bool attacker_turn, statistics::attack_context &stats) {
    unit_info &attacker = * (attacker_turn ? &a_ : &d_), &defender = * (
        attacker_turn ? &d_ : &a_);
    const battle_context_unit_stats
        *&attacker_stats = * (attacker_turn ? &a_stats_ : &d_stats_), *&defender_stats = * (
            attacker_turn ? &d_stats_ : &a_stats_);
    ...
    if (ran_results) {
        ...
        if (results_hits != hits) {
            errbuf_ << "SYNC: In attack " << a_.dump() << " vs " << d_.dump()
                    ...
                    << (hits ? "successful" : "unsuccessful") << " random number: " << ran_num
    ...
    if (hits && attacker_stats->drains) {
        drains_damage = damage_done * attacker_stats->drain_percent / 100
            + attacker_stats->drain_constant;
        // don't drain so much that the attacker gets more than his maximum hitpoints
        drains_damage = std::min<int>(
            drains_damage,
            attacker.get_unit().max_hitpoints() - attacker.get_unit().hitpoints());
    ...
    if (update_display_) {
        ...
        if (hits) {
            ...
            if (attacker_stats->poisons && !defender_unit.get_state(unit::STATE_POISONED)) {
                float_text
                    << (defender_unit.gender() == unit_race::FEMALE ? _("female^poisoned") :
                                                                      _("poisoned"))
                    << '\n';
            }
            ...
            if (attacker_stats->slows && !defender_unit.get_state(unit::STATE_SLOWED)) {
                float_text
                    << (defender_unit.gender() == unit_race::FEMALE ? _("female^slowed") :
                                                                      _("slowed"))
                    << '\n';
            }
            ...
            if (attacker_stats->petrifies) {
                float_text
                    << (defender_unit.gender() == unit_race::FEMALE ? _("female^petrified") :
                                                                      _("petrified"))
                    << '\n';
            }
        }

        unit_display::unit_attack(attacker.loc_, defender.loc_, damage, *attacker_stats->weapon,
                                  defender_stats->weapon, abs_n, float_text.str(),
                                  drains_damage, "");
    }
...
void attack::unit_killed(unit_info& attacker,
    ...
    config dat;
    config a_weapon_cfg =
        attacker_stats->weapon && attacker.valid() ? attacker_stats->weapon->get_cfg() :
                                                     config();
    config d_weapon_cfg =
        defender_stats->weapon && defender.valid() ? defender_stats->weapon->get_cfg() :
                                                     config();
    if (a_weapon_cfg["name"].empty()) a_weapon_cfg["name"] = "none";
    if (d_weapon_cfg["name"].empty()) d_weapon_cfg["name"] = "none";
    ...
    if ( !attacker.valid()) {
        ...
        unit_display::unit_die(defender.loc_, defender.get_unit(), attacker_stats->weapon,
                               defender_stats->weapon, attacker.loc_, &attacker.get_unit());
    }
...
void attack::perform() {
    ...
    a_.get_unit().set_movement(
        a_.get_unit().movement_left() - a_.get_unit().attacks()[a_.weapon_].movement_used(),
        true);
    ...
    statistics::attack_context attack_stats(a_.get_unit(), d_.get_unit(),
...

    {
        ...
        const double attacker_inflict = static_cast<double>(d_.get_unit().hitpoints())
            - defender.average_hp();
        const double defender_inflict = static_cast<double>(a_.get_unit().hitpoints())
            - attacker.average_hp();
    ...
    unit_display::unit_draw_weapon(a_.loc_, a_.get_unit(), a_stats_->weapon, d_stats_->weapon,
                                   d_.loc_, &d_.get_unit());
    ...
    unit_display::unit_sheath_weapon(a_.loc_, a_.valid() ? &a_.get_unit() : NULL,
                                     a_stats_->weapon, d_stats_->weapon, d_.loc_,
                                     d_.valid() ? &d_.get_unit() : NULL);
...
void advance_unit(map_location loc,
    ...
    unit new_unit =
        use_amla ? get_amla_unit( *u, *mod_option) : get_advanced_unit( *u, advance_to);
    if ( !use_amla) {
    ...
    // "sighted" event(s).
    clearer.fire_events();
    if (u.valid()) actions::actor_sighted( *u, &not_seeing);
...
int combat_modifier(const map_location &loc, unit_type::ALIGNMENT alignment, bool is_fearless) {
...
int generic_combat_modifier(int lawful_bonus,
    ...
    if (is_fearless) bonus = std::max<int>(bonus, 0);
...
bool backstab_check(const map_location& attacker_loc,
    ...
    if (teams[defender->side() - 1].is_enemy(opp->side())) return true;  // Defender and opposite are enemies
