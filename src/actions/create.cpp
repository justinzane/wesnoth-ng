...
struct castle_cost_calculator: pathfind::cost_calculator {
        castle_cost_calculator(const gamemap& map, const team & view_team) :
            map_(map), viewer_(view_team), use_shroud_(view_team.uses_shroud()) {
        }

        virtual double cost(const map_location& loc, const double) const {
            if ( !map_.is_castle(loc)) return 10000;

            if (use_shroud_ && viewer_.shrouded(loc)) return 10000;
...
void unit_creator::add_unit(const config &cfg, const vconfig* vcfg) {
    ...
    if (recall_list_element == recall_list.end()) {
        ...
        boost::scoped_ptr<unit> temp_unit(
            new unit(temp_cfg, true, resources::state_of_game, vcfg));
...
bool can_recruit_from(const map_location& leader_loc, int side) {
    ...
    if ( !map.is_keep(leader_loc)) return false;
...
bool can_recruit_on(const map_location& leader_loc, const map_location& recruit_loc, int side) {
    ...
    if ( !map.is_castle(recruit_loc)) return false;
    ...
    if (view_team.shrouded(recruit_loc)) return false;

    if (get_visible_unit(recruit_loc, view_team) != NULL) return false;
    ...
    pathfind::plain_route rt = pathfind::a_star_search(leader_loc, recruit_loc,
                                                       map.w() + map.h(), &calc, map.w(),
                                                       map.h());
...
const std::set<std::string> get_recruits(int side, const map_location &recruit_loc) {
    ...
    if ( !leader_in_place) {
        ...
        for (; u != u_end; ++u) {
            ...
            if ( ! (u->can_recruit() && u->side() == side)) continue;
...
void add_leader_filtered_recalls(const unit& leader,
                                 ...
                                 std::set<std::string>* already_added = NULL) {
    ...
    BOOST_FOREACH(const unit& recall_unit, recall_list) {
        ...
        std::string underlying_id = recall_unit.underlying_id();
        if (!already_added || already_added->count(underlying_id) == 0) {
            ...
            scoped_recall_unit this_unit("this_unit",
                                         save_id,
                                         &recall_unit - &recall_list[0]);

            if (recall_unit.matches_filter(vconfig(leader.recall_filter()),
                                           map_location::null_location)) {
                result.push_back( &recall_unit);
                if (already_added != NULL) { already_added->insert(underlying_id); }
            }
        }
    }
}
...
const std::vector<const unit*> get_recalls(int side, const map_location &recall_loc) {
    ...
    if ( allow_local )
    {
        ...
        std::set<std::string> valid_local_recalls;
 ...
 */
RECRUIT_CHECK check_unit_recall_location(const unit & recaller,
                                         const unit & recall_unit,
                                         const map_location & preferred,
                                         map_location & alternative) {
    // Make sure the unit can actually recall.
    if ( !recaller.can_recruit()) return RECRUIT_NO_LEADER;
    ...
    if ( !resources::game_map->is_keep(recaller.get_location())) return RECRUIT_NO_KEEP_LEADER;
    ...
    if ( !permissible.valid()) return RECRUIT_NO_VACANCY;
...
RECRUIT_CHECK check_recall_location(const int side,
    ...
    if (resources::units->count(recall_location) != 0) check_location =
        map_location::null_location;

    // If the check location is not valid, we will never get an "OK" result.
    RECRUIT_CHECK const goal_result =
        check_location.valid() ? RECRUIT_OK : RECRUIT_ALTERNATE_LOCATION;
    ...
    for (u = units.begin(); best_result < goal_result && u != u_end; ++u) {
        ...
        RECRUIT_CHECK current_result = check_unit_recall_location( *u, unit_recall,
                                                                  check_location, alternative);

        // If this is not an improvement, proceed to the next unit.
        if (current_result <= best_result) continue;
        best_result = current_result;

        // If we have a viable recaller, record its location.
        if (current_result >= RECRUIT_ALTERNATE_LOCATION) recall_from = u->get_location();
    }
...
std::string find_recall_location(const int side,
                                 map_location& recall_location,
                                 map_location& recall_from,
                                 const unit &unit_recall) {
 ...
 */
RECRUIT_CHECK check_unit_recruit_location(const unit & recruiter,
                                          const std::string & unit_type,
                                          const map_location & preferred,
                                          map_location & alternative) {
    // Make sure the unit can actually recruit.
    if ( !recruiter.can_recruit()) return RECRUIT_NO_LEADER;

    if ( !unit_type.empty()) {
        // Make sure the specified type is in the unit's recruit list.
        if ( !util::contains(recruiter.recruits(), unit_type)) return RECRUIT_NO_ABLE_LEADER;
    }

    // Make sure the unit is on a keep.
    if ( !resources::game_map->is_keep(recruiter.get_location())) return RECRUIT_NO_KEEP_LEADER;
...
RECRUIT_CHECK check_recruit_location(const int side,
    ...
    // If the specified location is occupied, proceed as if no location was specified.
    if (resources::units->count(recruit_location) != 0) check_location =
        map_location::null_location;
    ...
    for (u = units.begin(); best_result < goal_result && u != u_end; ++u) {
        ...
        RECRUIT_CHECK current_result = check_unit_recruit_location( *u, check_type,
                                                                   check_location, alternative);
        ...
        if (current_result >= RECRUIT_ALTERNATE_LOCATION) recruited_from = u->get_location();
    }
...
std::string find_recruit_location(const int side,
                                  map_location& recruit_location,
                                  map_location& recruited_from,
                                  const std::string& unit_type) {
...
void recruit_checksums(const unit &new_unit, bool wml_triggered) {
    ...
    if (ran_results != NULL) {
        ...
        if (rc != checksum) {
            ...
            error_msg << "SYNC: In recruit " << new_unit.type_id() << ": has checksum "
                      << checksum << " while datasource has checksum " << rc << "\n";
...
const map_location & find_recruit_leader(int side,
    ...
    if (leader != units.end() && leader->can_recruit() && leader->side() == side
        && can_recruit_on( *leader, recruit_location)) return leader->get_location();
...
bool place_recruit(const unit &u,
                   const map_location &recruit_location,
                   const map_location& recruited_from,
                   int cost,
                   bool is_recall,
...
void recruit_unit(const unit_type & u_type,
    ...
    if (use_recorder) recorder.add_recruit(u_type.id(), loc, from);
    ...
    if (use_undo) {
        ...
        if (mutated || u_type.genders().size() > 1 || new_unit.type().has_random_traits()) {
            resources::undo_stack->clear();
        }
    }

    // Update the screen.
    if (resources::screen != NULL) resources::screen->invalidate_game_status();
    // Other updates were done by place_recruit().

    // Record a checksum so the replay can be verified.
    if (use_recorder) recorder.add_checksum_check(loc);
}
...
bool recall_unit(const std::string & id,
    ...
    if (recall_it == recall_list.end()) return false;

    // Record this before actually recalling.
    if (use_recorder) recorder.add_recall(id, loc, from);
    ...
    // Place the recall.
    bool mutated = place_recruit(recall, loc, from, current_team.recall_cost(), true, show);
