...
void clearer_info::write(config & cfg) const {
    ...
    if (slowed) cfg.add_child("status")["slowed"] = true;
...
struct shroud_clearer::sight_data {
        sight_data(std::string viewed_id,
 ...
 */
inline void shroud_clearer::record_sighting(const unit & seen,
                                            const map_location & seen_loc,
                                            std::string sighter_id,
                                            const map_location & sighter_loc) {
    sightings_.push_back(sight_data(seen.underlying_id(), seen_loc, sighter_id, sighter_loc));
}
...
shroud_clearer::shroud_clearer() :
    jamming_(), sightings_(), view_team_(NULL) {
}
        ...
        if (sight_it.valid()) {
            ...
            if ( !sight_it->get_state(unit::STATE_PETRIFIED)) {
                if (tm.is_enemy(sight_it->side())) {
                    ++enemy_count;
                    if (spectator) spectator->add_seen_enemy(sight_it);
                } else {
                    ++friend_count;
                    if (spectator) spectator->add_seen_friend(sight_it);
                }
            }
        }
    }
...
bool shroud_clearer::clear_unit(const map_location &view_loc,
    ...
    if ( !instant && resources::screen) resources::screen->draw(true);

    bool cleared_something = false;
    // Dummy variables to make some logic simpler.
    size_t enemies = 0, friends = 0;
    if (enemy_count == NULL) enemy_count = &enemies;
    if (friend_count == NULL) friend_count = &friends;
    ...
    BOOST_FOREACH (const pathfind::paths::step &dest, sight.destinations) {
        ...
        if (clear_loc(view_team, dest.curr, view_loc, real_loc, viewer_id, !known, *enemy_count,
                      *friend_count, spectator)) cleared_something = true;
    }
    ...
    BOOST_FOREACH (const map_location &dest, sight.edges) {
        ...
        if (clear_loc(view_team, dest, view_loc, real_loc, viewer_id, !known, *enemy_count,
                      *friend_count, spectator)) cleared_something = true;
    }
...
bool shroud_clearer::clear_unit(const map_location &view_loc,
    ...
    return clear_unit(view_loc, view_team, viewer.underlying_id(), viewer.vision(),
                      viewer.get_state(unit::STATE_SLOWED), viewer.movement_type().get_vision(),
                      viewer.get_location(), known_units, enemy_count, friend_count, spectator,
                      instant);
}
...
bool shroud_clearer::clear_unit(const map_location &view_loc,
    ...
    const map_location & real_loc =
        find_it == resources::units->end() ? map_location::null_location :
                                             find_it->get_location();

    return clear_unit(view_loc, view_team, viewer.underlying_id, viewer.sight_range,
                      viewer.slowed, viewer.costs, real_loc, NULL, NULL, NULL, NULL, instant);
}
...
bool shroud_clearer::clear_unit(const map_location &view_loc,
    ...
    if ( !viewing_team.fog_or_shroud()) return false;
    ...
    if (invalidate) invalidate_after_clear();
...
bool shroud_clearer::clear_dest(const map_location &dest, const unit &viewer) {
    ...
    const std::string viewer_id = viewer.underlying_id();

    // Clear the destination.
    bool cleared_something = clear_loc(viewing_team, dest, dest, real_loc, viewer_id, true,
                                       enemies, friends);
    ...
    for (int i = 0; i != 6; ++i)
        if (clear_loc(viewing_team, adjacent[i], dest, real_loc, viewer_id, true, enemies,
                      friends)) cleared_something = true;

    if (cleared_something) invalidate_after_clear();
...
bool shroud_clearer::fire_events() {
    ...
    if (sightings_.empty()) return false;
    ...
    BOOST_FOREACH (const sight_data & event, sight_list) {
        ...
        {	// Raise the event based on the latest data.
            ...
            raise(sighted_str, entity_location(event.seen_loc, event.seen_id),
 ...
 */
{
    ...
    for (size_t i = 0; i != teams_size; ++i)
        if (needs_event[i]) create_jamming_map(jamming_cache[i], teams[i]);
    ...
    BOOST_FOREACH (const unit & viewer, *resources::units) {
        ...
        if (needs_event[index] && distances[index] != 0) {
            if (can_see(viewer, target_loc, &jamming_cache[index])) {
                ...
                size_t viewer_distance = distance_between(target_loc, viewer.get_location());
...
void recalculate_fog(int side) {
    ...
    BOOST_FOREACH (const unit &u, *resources::units) {
        ...
        if ( !tm.fogged(u_location)) visible_locs.insert(u_location);
    }
    ...
    BOOST_FOREACH(const unit &u, *resources::units) {
        if (u.side() == side) clearer.clear_unit(u.get_location(), u, tm, &visible_locs);
    }
    // Update the screen.
...
bool clear_shroud(int side, bool reset_fog, bool fire_events) {
    team &tm = ( *resources::teams)[side - 1];
    if ( !tm.uses_shroud() && !tm.uses_fog()) return false;

    bool result = false;

    shroud_clearer clearer;
    BOOST_FOREACH(const unit &u, *resources::units) {
        if (u.side() == side) result |= clearer.clear_unit(u.get_location(), u, tm);
    }
    // Update the screen.
    if (result) clearer.invalidate_after_clear();
