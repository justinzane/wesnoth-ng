...
inline unsigned swarm_blows(unsigned min_blows,
                            unsigned max_blows,
                            unsigned hp,
                            unsigned max_hp) {
    return
...
struct battle_context_unit_stats {
        ...
        weapon(NULL),
        attack_num(0),
        is_attacker(true),// These are not used in attack prediction.
        is_poisoned(false),
        is_slowed(slowed),
        slows(slows),
        drains(drain),
...
class battle_context {
    public:
        ...
        battle_context(const battle_context_unit_stats &att,
        ...
        bool better_attack(class battle_context &that,
                           double harm_weight);
