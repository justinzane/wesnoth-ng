/**
 * @file 		rndr_mgr.cpp is part of wesnoth-ng
...
std::pair<int, int>& rndr_mgr::get_window_size() {
    SDL_GetWindowSize(wind_, &(win_size_.first), &(win_size_.second));
    return win_size_;
}

void rndr_mgr::blit_surf(SDL_Surface* surf, SDL_Rect* src_rect, SDL_Rect* dst_rect) {
    SDL_BlitSurface(surf, src_rect, surf_, dst_rect);
    render();
}

void rndr_mgr::render() {
