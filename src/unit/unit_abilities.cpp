...
bool affects_side(const config& cfg,
                  const std::vector<team>& teams,
                  size_t side,
                  size_t other_side) {
    if (side == other_side) return cfg["affect_allies"].to_bool(true);
...
bool unit::get_ability_bool(const std::string& tag_name, const map_location& loc) const {
    ...
    for (int i = 0; i != 6; ++i) {
        ...
        if (it == units.end() || it->incapacitated()) continue;
        ...
        if ( !adj_abilities) continue;
        BOOST_FOREACH(const config &j, adj_abilities.child_range(tag_name)) {
            if (affects_side(j, teams_manager::get_teams(), side(), it->side()) && it
                ->ability_active(tag_name, j, adjacent[i])
                && ability_affects_adjacent(tag_name, j, i, loc)) return true;
        }
    }
 ...
 */
inline const config::attribute_value & default_value(const config & cfg,
 ...
 */
inline const config::attribute_value & gender_value(const config & cfg,
                                                    unit_race::GENDER gender,
                                                    const std::string & male_key,
                                                    const std::string & female_key,
                                                    const std::string & default_key) {
    return default_value(cfg, gender == unit_race::MALE ? male_key : female_key, default_key);
}
}
...
std::vector<boost::tuple<t_string, t_string, t_string> > unit::ability_tooltips(std::vector<bool> *active_list) const {
    ...
    if (active_list) active_list->clear();
    ...
    BOOST_FOREACH(const config::any_child &ab, abilities.all_children_range()) {
        if ( !active_list || ability_active(ab.key, ab.cfg, loc_)) {
            t_string const &name = gender_value(ab.cfg, gender_, "name", "female_name", "name")
                .t_str();

            if ( !name.empty()) {
                res.push_back(
                    boost::make_tuple(
                        ab.cfg["name"].t_str(), name,
                        legacy::ability_description(ab.cfg["description"].t_str())));
                if (active_list) active_list->push_back(true);
            }
        } else {
            // See if an inactive name was specified.
            config::attribute_value const &inactive_value = gender_value(ab.cfg, gender_,
                                                                         "name_inactive",
                                                                         "female_name_inactive",
                                                                         "name_inactive");
            t_string const &name =
                !inactive_value.blank() ?
...
bool unit::ability_active(const std::string& ability,
    ...
    assert(
        resources::units && resources::game_map && resources::teams && resources::tod_manager);

    if (const config &afilter = cfg.child("filter"))
        if ( !matches_filter(vconfig(afilter), loc, illuminates)) return false;
    ...
    BOOST_FOREACH(const config &i, cfg.child_range("filter_adjacent")) {
        BOOST_FOREACH(const std::string &j, utils::split(i["adjacent"])) {
            ...
            if (index == map_location::NDIRECTIONS) continue;
    ...
    BOOST_FOREACH(const config &i, cfg.child_range("affect_adjacent")) {
        ...
        if (std::find(dirs.begin(), dirs.end(), adjacent_names[dir]) != dirs.end()) {
            if (const config &filter = i.child("filter")) {
                if (matches_filter(vconfig(filter), loc, illuminates)) return true;
            } else
                return true;
        }
    }
 ...
 *           false otherwise.
 */
bool get_special_children(std::vector<const config*>& result,
                          const config& parent,
                          const std::string& id,
...
bool attack_type::get_special_bool(const std::string& special, bool simple_check) const {
    ...
    if (const config &specials = cfg_.child("specials")) {
        ...
        if (get_special_children(list, specials, special, simple_check)) return true;
        ...
        for (std::vector<const config*>::iterator i = list.begin(), i_end = list.end();
            i != i_end; ++i) {
            if (special_active( * *i, AFFECT_SELF)) return true;
        }
    }

    // Skip checking the opponent's attack?
    if (simple_check || !other_attack_) return false;
    ...
    if (const config &specials = other_attack_->cfg_.child("specials")) {
        ...
        for (std::vector<const config*>::iterator i = list.begin(), i_end = list.end();
            i != i_end; ++i) {
            if (other_attack_->special_active( * *i, AFFECT_OTHER)) return true;
        }
    }
    return false;
}
...
unit_ability_list attack_type::get_specials(const std::string& special) const {
    ...
    if (const config &specials = cfg_.child("specials")) {
        BOOST_FOREACH(const config &i, specials.child_range(special)) {
            if (special_active(i, AFFECT_SELF)) res.push_back(unit_ability( &i, self_loc_));
        }
    }
 ...
 */
std::vector<std::pair<t_string, t_string> > attack_type::special_tooltips(std::vector<bool> *active_list) const {
    ...
    BOOST_FOREACH(const config::any_child &sp, specials.all_children_range()) {
        if ( !active_list || special_active(sp.cfg, AFFECT_EITHER)) {
            ...
            if ( !name.empty()) {
                res.push_back(
                    std::make_pair(name,
                                   legacy::ability_description(sp.cfg["description"].t_str())));
                if (active_list) active_list->push_back(true);
            }
        } else {
            ...
            if ( !name.empty()) {
                res.push_back(
                    std::make_pair(
                        name,
                        legacy::ability_description(
                            default_value(sp.cfg, "description_inactive", "description").t_str())));
                active_list->push_back(false);
            }
        }
    }
...
std::string attack_type::weapon_specials(bool only_active, bool is_backstab) const {
    ...
    BOOST_FOREACH(const config::any_child &sp, specials.all_children_range()) {
        if (only_active && !special_active(sp.cfg, AFFECT_EITHER, is_backstab)) continue;
...
void attack_type::modified_attacks(bool is_backstab,
    ...
    if ( !swarm_specials.empty()) {
        ...
        max_attacks = std::max<int>(
            0, swarm_specials.highest("swarm_attacks_max", attacks_value).first);
    } else {
...
bool special_affects_opponent(const config& special, bool is_attacker) {
    ...
    if (apply_to.empty()) return false;
    if (apply_to == "both") return true;
    if (apply_to == "opponent") return true;
    if (is_attacker && apply_to == "defender") return true;
    if ( !is_attacker && apply_to == "attacker") return true;
    return false;
}
...
bool special_affects_self(const config& special, bool is_attacker) {
    ...
    std::string const &apply_to = special["apply_to"];
    if (apply_to.empty()) return true;
    if (apply_to == "both") return true;
    if (apply_to == "self") return true;
    if (is_attacker && apply_to == "attacker") return true;
    if ( !is_attacker && apply_to == "defender") return true;
    return false;
}
...
static bool special_unit_matches(const unit_map::const_iterator & un_it,
    ...
    if ( !un_it.valid() || !un_it->matches_filter(vconfig(filter_child), loc)) return false;

    // Check for a weapon match.
    if (const config & filter_weapon = filter_child.child("filter_weapon")) {
        if ( !weapon || !weapon->matches_filter(filter_weapon)) return false;
    }
...
bool attack_type::special_active(const config& special,
    ...
    // Backstab check
    if ( !include_backstab) if (special["backstab"].to_bool()) return false;

    // Does this affect the specified unit?
    if (whom == AFFECT_SELF) {
        if ( !special_affects_self(special, is_attacker_)) return false;
    }
    if (whom == AFFECT_OTHER) {
        if ( !special_affects_opponent(special, is_attacker_)) return false;
    }

    // Is this active on attack/defense?
    const std::string & active_on = special["active_on"];
    if ( !active_on.empty()) {
        if (is_attacker_ && active_on != "offense") return false;
        if ( !is_attacker_ && active_on != "defense") return false;
    }
    ...
    if ( !special_unit_matches(self, self_loc_, this, special, "filter_self")) return false;
    ...
    BOOST_FOREACH(const config &i, special.child_range("filter_adjacent")) {
        BOOST_FOREACH(const std::string &j, utils::split(i["adjacent"])) {
            ...
            if (index == map_location::NDIRECTIONS) continue;
            unit_map::const_iterator unit = units.find(adjacent[index]);
            if (unit == units.end() || !unit->matches_filter(vconfig(i), adjacent[index]))
                return false;
        }
    }
...
ability_effect::ability_effect(const unit_ability_list& list, int def, bool backstab) :
    ...
    BOOST_FOREACH (const unit_ability & ability, list) {
        ...
        if ( !backstab && cfg["backstab"].to_bool()) continue;
        if ( !filter_base_matches(cfg, def)) continue;
        ...
        if (const config::attribute_value *v = cfg.get("add")) {
            int add = *v;
            std::map<std::string, individual_effect>::iterator add_effect = values_add.find(
                effect_id);
        ...
        if (const config::attribute_value *v = cfg.get("sub")) {
            int sub = - *v;
            std::map<std::string, individual_effect>::iterator sub_effect = values_add.find(
                effect_id);
        ...
        if (const config::attribute_value *v = cfg.get("multiply")) {
            ...
            std::map<std::string, individual_effect>::iterator mul_effect = values_mul.find(
                effect_id);
