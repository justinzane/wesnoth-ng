...
// Instance Members/Methods -------------------------------------------------------------------
    ...
    public:
        ...
        void set_can_recurit(bool canrecruit) {
        ...
        void toggle_hold_position() {
            ...
            if (hold_position_) end_turn_ = true;
        }
        ...
        void toggle_user_end_turn() {
            end_turn_ = !end_turn_;
            if ( !end_turn_) hold_position_ = false;
        }
        ...
        bool matches_filter(const vconfig& cfg, const map_location& loc, bool use_flat_tod =
            false) const;
        ...
        int damage_from(const attack_type& attack,
                        bool attacker,
        ...
        int resistance_against(const std::string& damage_name,
                               bool attacker,
                               const map_location& loc) const;
        int resistance_against(const attack_type& damage_type,
                               bool attacker,
                               const map_location& loc) const {
        ...
        void add_modification(const std::string& type, const config& modification, bool no_add =
            false);
        ...
        unit_ability_list get_abilities(const std::string &tag_name,
                                        const map_location& loc) const;
        unit_ability_list get_abilities(const std::string &tag_name) const {
            return get_abilities(tag_name, loc_);
        }
        /** Tuple of: neutral ability name, gendered ability name, description */
        std::vector<boost::tuple<t_string, t_string, t_string> > ability_tooltips(std::vector<
            bool> *active_list = NULL) const;
        ...
        bool is_visible_to_team(team const& team,
                                bool const see_all = true,
                                gamemap const& map = *resources::game_map) const;
    ...
    private:
         ...
         */
        bool ability_active(const std::string& ability,
                            const config& cfg,
                            const map_location& loc) const;
        bool ability_affects_adjacent(const std::string& ability,
                                      const config& cfg,
                                      int dir,
                                      const map_location& loc) const;
        bool ability_affects_self(const std::string& ability,
                                  const config& cfg,
                                  const map_location& loc) const;
        bool resistance_filter_matches(const config& cfg,
                                       bool attacker,
 ...
 * scope.
 */
struct unit_movement_resetter: private boost::noncopyable {
...
std::vector<unit>::iterator find_if_matches_id(std::vector<unit> &unit_list,
                                               const std::string &unit_id);
/// Used to find units in vectors by their ID. (Convenience wrapper)
std::vector<unit>::const_iterator find_if_matches_id(const std::vector<unit> &unit_list,
                                                     const std::string &unit_id);
/// Used to erase units from vectors by their ID. (Convenience wrapper)
std::vector<unit>::iterator erase_if_matches_id(std::vector<unit> &unit_list,
                                                const std::string &unit_id);
...
struct team_data {
        team_data() :
            units(0), upkeep(0), villages(0), expenses(0), net_income(0), gold(0), teamname() {
        }
...
struct temporary_unit_mover {
        temporary_unit_mover(unit_map& m,
