/**
 * @file src/editor/action/action_label.cpp
 * @project The Battle for Wesnoth NG - https://github.com/justinzane/wesnoth-ng
 * @brief 
 * @authors 
 * @copyright Copyright (C) 2010 - 2013 by Fabian Mueller <fabianmueller5@gmx.de>
...
void editor_action_label::perform_without_undo(map_context& mc) const {
    mc.get_labels().set_label(loc_, text_, team_name_, color_, visible_fog_, visible_shroud_,
                              immutable_);
}
...
editor_action* editor_action_label_delete::perform(map_context& mc) const {
    ...
    undo.reset(
        new editor_action_label(loc_, deleted->text(), deleted->team_name(), deleted->color(),
                                deleted->visible_in_fog(), deleted->visible_in_shroud(),
                                deleted->immutable()));
