...
editor_map_load_exception wrap_exc(const char* type,
...
editor_map::editor_map(const config& terrain_cfg) :
    gamemap(terrain_cfg, gamemap::default_map_header), selection_() {
}

editor_map::editor_map(const config& terrain_cfg, const std::string& data) :
    gamemap(terrain_cfg, data), selection_() {
    sanity_check();
}
...
editor_map::editor_map(const config& terrain_cfg,
    ...
    gamemap(
        terrain_cfg,
        gamemap::default_map_header + t_translation::write_game_map(
...
bool editor_map::set_selection(const std::set<map_location>& area) {
    ...
    BOOST_FOREACH(const map_location& loc, area) {
        if ( !add_to_selection(loc)) return false;
    }
    return true;
}
...
void editor_map::resize(int width,
                        int height,
                        int x_offset,
                        int y_offset,
                        const t_translation::t_terrain & filler) {
...
gamemap editor_map::mask_to(const gamemap& target) const {
    if (target.w() != w() || target.h() != h()) {
        throw editor_action_exception(
            _("The size of the target map is different from the current map"));
    }
    gamemap mask(target);
    map_location iter;
...
t_translation::t_list editor_map::clone_column(int x, const t_translation::t_terrain & filler) {
    ...
    for (int y = 0; y < h; ++y) {
        column[y] = filler != t_translation::NONE_TERRAIN ? filler : tiles_[x][y];
    ...
    for (int y = 0; y < count; ++y) {
        for (int x = 0; x < static_cast<int>(tiles_.size()); ++x) {
            t_translation::t_terrain terrain =
                filler != t_translation::NONE_TERRAIN ? filler : tiles_[x][0];
...
void editor_map::expand_bottom(int count, const t_translation::t_terrain & filler) {
    ...
    for (int y = 0; y < count; ++y) {
        for (int x = 0; x < static_cast<int>(tiles_.size()); ++x) {
            t_translation::t_terrain terrain =
                filler != t_translation::NONE_TERRAIN ? filler : tiles_[x][h - 1];
            assert(terrain != t_translation::NONE_TERRAIN);
