...
/**
 * @brief A variable-expanding proxy for the config class.
 * @details This class roughly behaves as a constant config object, but automatically
 * expands variables.
 * @note When dealing with a vconfig, keep in mind its lifetime. By default,
 * vconfigs do not maintain a copy their data; if you need a vconfig to stick around,
...
class scoped_xy_unit: public scoped_wml_variable {
    public:
        scoped_xy_unit(const std::string& var_name,
                       const int x,
                       const int y,
                       const unit_map& umap) :
...
class scoped_recall_unit: public scoped_wml_variable {
    public:
        scoped_recall_unit(const std::string& var_name,
...
struct variable_info {
         ...
         */
        enum TYPE {
            TYPE_SCALAR,    //a Scalar variable resolves to a t_string attribute of *vars
        ...
        variable_info(const std::string& varname,
