...
play_controller::play_controller(const config& level,
    ...
    try {
        ...
        std::cout << "JZ play_controller caught a std::logic_error trying to init(video)"
                  << "\n\t" << e.what() << "\n";
    } catch (...) {
        clear_resources();
        throw;
...
void play_controller::init(CVideo& video) {
    ...
    BOOST_FOREACH(const config &side, level_.child_range("side")) {
        ...
        if (first_human_team_ == -1) {
            ...
            if (controller == "human" && side["id"] == preferences::login()) {
                first_human_team_ = team_num;
            } else if (controller == "human") {
                first_human_team_ = team_num;
            }
        }
        team_builder_ptr tb_ptr = gamedata_.create_team_builder(side, save_id, teams_, level_,
                                                                map_, units_, snapshot,
                                                                gamestate_.replay_start());
        ++team_num;
...
void play_controller::save_game() {
    if (save_blocker::try_block()) {
        ...
        savegame::ingame_savegame save(gamestate_, *gui_, to_config(),
...
void play_controller::do_init_side(const unsigned int team_index, bool is_replay) {
    ...
    if ( !loading_game_ && turn() > 1) {
        ...
        int expense = side_upkeep(player_number_) - current_team.support();
        if (expense > 0) {
...
void play_controller::finish_side_turn() {
    ...
    for (unit_map::iterator uit = units_.begin(); uit != units_.end(); ++uit) {
        if (uit->side() == player_number_) uit->end_turn();
    }
...
bool play_controller::enemies_visible() const {
    ...
    for (unit_map::const_iterator u = units_.begin(); u != units_.end(); ++u)
        if (current_team().is_enemy(u->side()) && !gui_->fogged(u->get_location())) return true;
...
void play_controller::fire_wml_menu_item_event(const wml_menu_item &menu_item) {
    ...
    if (game_events::fire((std::string)event_name, (game_events::entity_location)menu_hex)) {
...
bool play_controller::execute_command(const hotkey::hotkey_command& cmd, int index) {
    ...
    if (command == hotkey::HOTKEY_WML && cmd.command.compare(0, prefixlen,
        ...
        if (iter != gs_wmi.end()) {
            ...
            if (name == iter->id()) {
                ...
                if (iter->can_show(hex)) {
                    if ( ( !iter->needs_select() || gamedata_.last_selected.valid())) {
...
bool play_controller::can_execute_command(const hotkey::hotkey_command& cmd, int index) const {
    if (index >= 0) {
        unsigned i = static_cast<unsigned>(index);
        if ( (i < savenames_.size() && !savenames_[i].empty()) || (i < wml_commands_.size()
            && wml_commands_[i] != NULL)) {
            return true;
        }
    }
    switch (cmd.id) {
        ...
        case hotkey::HOTKEY_RENAME_UNIT:
            return !events::commands_disabled && menu_handler_.current_unit() != units_.end()
    ...
    switch (mode) {
        case gui::TEXTBOX_SEARCH: {
            BOOST_FOREACH(const unit &u, units_) {
                ...
                if ( !gui_->fogged(loc) && ! (teams_[gui_->viewing_team()].is_enemy(u.side())
                    && u.invisible(loc))) dictionary.insert(u.name());
            }
        ...
        case gui::TEXTBOX_MESSAGE: {
            BOOST_FOREACH(const team& t, teams_) {
                if ( !t.is_empty()) dictionary.insert(t.current_player());
            }
...
std::string play_controller::get_unique_saveid(const config& cfg,
                                               std::set<std::string>& seen_save_ids) {
...
int play_controller::find_human_team_before(const size_t team_num) const {
    if (team_num > teams_.size()) return -2;
        ...
        if (new_path_turns != mouse_handler_.get_path_turns()) {
            ...
            if (u != units_.end()) {
                ...
                mouse_handler_.set_current_paths(
                    pathfind::paths( *u, false, true, teams_[gui_->viewing_team()],
                                    mouse_handler_.get_path_turns()));

                gui_->highlight_reach(mouse_handler_.current_paths());
            } else {
                mouse_handler_.select_hex(mouse_handler_.get_selected_hex(), false, false,
                                          false);
            }
    ...
    for (unsigned int i = 0; i < items.size(); ++i) {
        if (items[i] == "AUTOSAVES") {
            ...
            for (unsigned int turn = this->turn(); turn != 0; turn--) {
                std::string name = gamestate_.classification().label + "-" + _("Auto-Save")
                                   + lexical_cast<std::string>(turn);
                if (savegame::save_game_exists(name, preferences::compress_saves())) {
                    if (preferences::compress_saves()) {
                        newsaves.push_back(
                            name + (preferences::bzip2_savegame_compression() ? ".bz2" : ".gz"));
                    } else {
            ...
            if (savegame::save_game_exists(start_name, preferences::compress_saves())) {
                if (preferences::compress_saves()) {
                    newsaves.push_back(
                        start_name + (preferences::bzip2_savegame_compression() ? ".bz2" : ".gz"));
                } else {
    ...
    for (unsigned int i = 0; i < items.size(); ++i) {
        if (items[i] == "wml") {
            ...
            for (wmi_container::const_iterator itor = gs_wmi.begin();
                itor != gs_wmi.end() && newitems.size() < MAX_WML_COMMANDS; ++itor) {
...
bool play_controller::in_context_menu(hotkey::HOTKEY_COMMAND command) const {
    switch (command) {
        ...
        case hotkey::HOTKEY_RECALL: {
            ...
            if ( !resources::game_map->is_keep(last_hex) && !resources::game_map->is_castle(
                last_hex)) return false;
            ...
            if (leader != units_.end())
                ...
                for (leader = units_.begin(); leader != units_.end(); ++leader) {
                    if (leader->can_recruit() && leader->side() == viewing_side
                        && can_recruit_on( *leader, last_hex)) return true;
                }
...
hotkey::ACTION_STATE play_controller::get_action_state(hotkey::HOTKEY_COMMAND command,
                                                       int /*index*/) const {
    switch (command) {
        case hotkey::HOTKEY_DELAY_SHROUD:
            return
                teams_[gui_->viewing_team()].auto_shroud_updates() ? hotkey::ACTION_OFF :
                                                                     hotkey::ACTION_ON;
        default:
...
void play_controller::set_defeat_music_list(const std::string& list) {
    ...
    if (defeat_music_.empty()) defeat_music_ = utils::split(game_config::default_defeat_music);
}
...
void play_controller::check_victory() {
    ...
    for (unit_map::const_iterator i = units_.begin(), i_end = units_.end(); i != i_end; ++i) {
        if (i->can_recruit()) {
    ...
    for (std::vector<team>::iterator tm_beg = teams_.begin(), tm = tm_beg, tm_end =
        teams_.end(); tm != tm_end; ++tm) {
        if (std::find(seen_leaders.begin(), seen_leaders.end(), tm - tm_beg + 1) == seen_leaders
            .end()) {
            tm->clear_villages();
