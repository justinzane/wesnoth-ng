...
/**@brief   Threading primitives wrapper for SDL_Thread.
 * @details This module defines primitives for wrapping C++ around SDL's threading interface */
namespace threading {
...
/**@brief Threading object.
 * @details This class defines threading objects. One such object represents a thread
 * and admits killing and joining on threads. Intended to be used for manipulating
 * threads instead of poking around with SDL_Thread calls.*/
class thread: //
    private boost::noncopyable
{
    public:
        /**@brief Construct a new thread to start executing the function pointed to by f.
         * @details The void* data will be passed to f, to facilitate passing of parameters to f.
        ...
        /** @brief Destroy the thread object. This is done by waiting on the
         thread with the join() operation, thus blocking until the
         thread object has finished its operation.*/
        ~thread();

        /**@brief Join (wait) on the thread to finish. When the thread finishes,
         * the function will return. calling wait() on an already killed thread is a no-op. */
        void join();
...
inline Uint32 get_current_thread_id() {
    return SDL_ThreadID();
}
// Binary mutexes.
//
...
class condition: private boost::noncopyable {
    public:
        ...
        enum WAIT_TIMEOUT_RESULT {
            WAIT_OK, WAIT_TIMED_OUT, WAIT_ERROR
        };
