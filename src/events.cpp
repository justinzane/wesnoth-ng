...
void handler::join() {
    // should not be in multiple event contexts
    if (has_joined_) { leave(); }
    //join self
    event_contexts.back().add_handler(this);
    has_joined_ = true;
...
void pump() {
    ...
    while (SDL_PollEvent( &temp_event)) {
        ++poll_count;
        /// @todo Verify that SDL_WINDOWEVENT is right.
    ...
    for (ev_it = events.begin(); ev_it != ev_end; ++ev_it) {
        ...
        switch (event.type) {
            ...
            case SDL_WINDOWEVENT: {
                SDL_WindowEvent& win_evt = reinterpret_cast<SDL_WindowEvent&>(event);
                /// @todo verify that this is proper for SDL2
                if ((win_evt.type & SDL_WINDOWEVENT_ENTER) != 0 ||
                    (win_evt.type & SDL_WINDOWEVENT_FOCUS_GAINED) != 0) {
            ...
            case SDL_WINDOWEVENT_RESIZED: {
                SDL_WindowEvent& win_evt = reinterpret_cast<SDL_WindowEvent&>(event);
                info.resize_dimensions.first = win_evt.data1;
                info.resize_dimensions.second = win_evt.data2;
                break;
            }
            ...
            case SDL_MOUSEBUTTONDOWN: {
                // always make sure cursor is displayed if mouse moves or the user clicks
                cursor::set_focus(true);
                if (event.button.button == SDL_BUTTON_LEFT) {
                    /** @todo Try to get these params from the OS, if possible so double-
                     * clicking will feel the same. */
                    ...
                    {
                        SDL_Event* user_evt = reinterpret_cast<SDL_Event*>( &event);
