...
const char LARGE_TEXT = '*', SMALL_TEXT = '`', BOLD_TEXT = '~', NORMAL_TEXT = '{',
    nullptr_MARKUP = '^', BLACK_TEXT = '}', GRAY_TEXT = '|', GOOD_TEXT = '@', BAD_TEXT = '#',
    GREEN_TEXT = '@', RED_TEXT = '#', COLOR_TEXT = '<', IMAGE = '&';

const std::string weapon = "<245,230,193>", weapon_details = "<166,146,117>", unit_type =
    "<245,230,193>", race = "<166,146,117>";

const SDL_Color weapon_color = { 245, 230, 193, 255 }, good_dmg_color = { 130, 240, 50, 255 },
    bad_dmg_color = { 250, 140, 80, 255 }, weapon_details_color = { 166, 146, 117, 255 },
    inactive_details_color = { 146, 146, 146, 255 }, inactive_ability_color = { 146, 146, 146,
                                                                                255 },
    unit_type_color = { 245, 230, 193, 255 }, race_color = { 166, 146, 117, 255 };
...
std::string del_tags(const std::string& text) {
    ...
    for (line = lines.begin(); line != lines.end(); ++line) {
        std::string::const_iterator i1 = line->begin(), i2 = line->end();
...
SDL_Rect draw_text(SDL_Surface* dst,
                   const SDL_Rect* area,
                   int size,
    ...
    for (;;) {
        ...
        if (i1 != i2) {
            ...
            const SDL_Rect rect = draw_text_line(dst, area, sz, col, new_string, x, y,
                                                 use_tooltips, text_style);
            if (rect.w > res.w) {
                res.w = rect.w;
            }
...
SDL_Rect draw_text(CVideo* gui,
                   ...
                   bool use_tooltips,
                   int style) {
    return draw_text(gui != nullptr ? gui->getSurface() : nullptr, area, size, color, txt, x, y,
                     use_tooltips, style);
}
...
bool is_cjk_char(const wchar_t c) {
    ...
    (ch >= 0x4e00 && ch < 0x9fcf) || (ch >= 0x3400 && ch < 0x4dbf)
    ...
    (ch >= 0xac00 && ch < 0xd7af)
    || (ch >= 0x1100 && ch <= 0x11ff)
    || (ch >= 0xa960 && ch <= 0xa97c) || (ch >= 0xd7b0 && ch <= 0xd7fb) ||
...
static void cut_word(std::string& line,
                     std::string& word,
                     int font_size,
                     int style,
                     int max_width) {
...
inline bool no_break_after(const wchar_t ch) {
     ...
     */
    ch == 0x2018 || ch == 0x201c || ch == 0x3008 || ch == 0x300a || ch == 0x300c || ch == 0x300e
    || ch == 0x3010 || ch == 0x3014 || ch == 0xff08 || ch == 0xff3b || ch == 0xff5b ||
...
inline bool no_break_before(const wchar_t ch) {
     ...
     */
    ch == 0x2019 || ch == 0x201d || ch == 0x2026 || ch == 0x3001 || ch == 0x3002 || ch == 0x3005
    || ch == 0x3009 || ch == 0x300b || ch == 0x300d || ch == 0x300f || ch == 0x3011
    || ch == 0x3015 || ch == 0x3041 || ch == 0x3043 || ch == 0x3045 || ch == 0x3047
    || ch == 0x3049 || ch == 0x3063 || ch == 0x3083 || ch == 0x3085 || ch == 0x3087
    || ch == 0x308e || ch == 0x309d || ch == 0x309e || ch == 0x30a1 || ch == 0x30a3
    || ch == 0x30a5 || ch == 0x30a7 || ch == 0x30a9 || ch == 0x30c3 || ch == 0x30e3
    || ch == 0x30e5 || ch == 0x30e7 || ch == 0x30ee || ch == 0x30f5 || ch == 0x30f6
    || ch == 0x30fb || ch == 0x30fc || ch == 0x30fd || ch == 0x30fe || ch == 0xff01
    || ch == 0xff09 || ch == 0xff0c || ch == 0xff0e || ch == 0xff1a || ch == 0xff1b
    || ch == 0xff1f
    || ch == 0xff3d
    || ch == 0xff5d
    ||
    ...
    ch == 0x31f0 || ch == 0x31f1 || ch == 0x31f2 || ch == 0x31f3 || ch == 0x31f4 || ch == 0x31f5
    || ch == 0x31f6 || ch == 0x31f7 || ch == 0x31f8 || ch == 0x31f9 || ch == 0x31fa
    || ch == 0x31fb || ch == 0x31fc || ch == 0x31fd || ch == 0x31fe || ch == 0x31ff ||
     ...
     */
    ch == 0x301c
    || ch == 0xff0d || ch == 0xff64 || ch == 0xff65 || ch == 0x3017 || ch == 0x301b
    || ch == 0x301e;
}

inline bool break_before(const wchar_t ch) {
    if (no_break_before(ch)) return false;

    return is_cjk_char(ch);
}

inline bool break_after(const wchar_t ch) {
    if (no_break_after(ch)) return false;
...
std::string word_wrap_text(const std::string& unwrapped_text,
                           int font_size,
                           int max_width,
                           int max_height,
                           int max_lines,
    ...
    while (1) {
        if (start_of_line) {
            line_width = 0;
            format_string.clear();
            while (ch != end && *ch < static_cast<wchar_t>(0x100) && is_format_char( *ch)
                   && !ch.next_is_end()) {
        ...
        if (current_word.empty() && ch == end) {
            ...
            if ( *ch == ' ' || *ch == '\n') {
                ...
                wchar_t previous = 0;
                for (;
                    ch != utils::utf8_iterator::end(unwrapped_text) && *ch != ' ' && *ch != '\n';
                    ++ch) {

                    if ( !current_word.empty() && break_before( *ch)
                        && !no_break_after(previous)) break;

                    if ( !current_word.empty() && break_after(previous)
                        && !no_break_before( *ch)) break;
...
SDL_Rect draw_wrapped_text(CVideo* gui,
                           const SDL_Rect* area,
                           int font_size,
                           const SDL_Color* color,
