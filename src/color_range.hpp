 ...
 * @details A color range definition is made of four reference RGB colors, used
 ...
 * @todo This should be refactored to produce colors with similar human visual
 * apparent brightnesses/saturations. Colors based on yellow-green are seen
 * as brighter than other colors; and, colors that only use one of the RGB colors
 * are generally less bright than colors that use two or three of the per-pixel
 * illuminants in display's additive color generation. <justin@justinzane.com>
 */
class color_range {
    public:
        /**
         ...
         */
        color_range(Uint32 mid, Uint32 max = 0x00FFFFFF, Uint32 min = 0x00000000, Uint32 rep =
            0x00808080) :
            mid_(mid), max_(max), min_(min), rep_(rep) {
        }
        ...
        /** Default constructor. */
        color_range() :
            mid_(0x00808080), max_(0x00FFFFFF), min_(0x00000000), rep_(0x00808080) {
        }
...
/**
 * @brief Convert comma separated string into rgb values.
 * Return false and empty result on error.
 */
bool string2rgb(const std::string& s, std::vector<Uint32>& result);

/**
 * @brief Convert comma separated string into argb values.
 * Return false and empty result on error.
 */
bool string2argb(const std::string& s, std::vector<Uint32>& result);
