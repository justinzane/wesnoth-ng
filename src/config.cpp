...
void config::append_children(const config &cfg)
{
	...
	foreach_ng(const any_child &value, cfg.all_children_range()) {
...
{
	append_children(cfg);
	foreach_ng(const attribute &v, cfg.values) {
...
void config::merge_children(const std::string& key)
{
	...
	foreach_ng(const config &cfg, child_range(key)) {
...
void config::merge_children_by_attribute(const std::string& key, const std::string& attribute)
{
	...
	clear_children(key);
	foreach_ng(const config_map::value_type &i, merged_children_map) {
...
void config::recursive_clear_value(const std::string& key)
{
	...
	foreach_ng(const any_child &value, all_children_range()) {
	...
	const child_map::iterator &pos, unsigned index)
{
	...
	unsigned found = 0;
	foreach_ng(child_pos &p, ordered_children)
	{
...
void config::apply_diff(const config& diff, bool track /* = false */)
{
	...
	if (const config &inserts = diff.child("insert")) {
		foreach_ng(const attribute &v, inserts.attribute_range()) {
			values[v.first] = v.second;
		}
	}

	if (const config &deletes = diff.child("delete")) {
		foreach_ng(const attribute &v, deletes.attribute_range()) {
			values.erase(v.first);
		}
	}

	foreach_ng(const config &i, diff.child_range("change_child"))
	{
		const size_t index = lexical_cast<size_t>(i["index"].str());
		foreach_ng(const any_child &item, i.all_children_range())
		{
			if (item.key.empty()) {
				continue;
			}
	...
	foreach_ng(const config &i, diff.child_range("insert_child"))
	{
		const size_t index = lexical_cast<size_t>(i["index"].str());
		foreach_ng(const any_child &item, i.all_children_range()) {
	...
	foreach_ng(const config &i, diff.child_range("delete_child"))
	{
		const size_t index = lexical_cast<size_t>(i["index"].str());
		foreach_ng(const any_child &item, i.all_children_range()) {
			if (!track) {
...
void config::merge_with(const config& c)
{
	...
	foreach_ng(const child_pos& pos, to_remove) {
...
bool config::matches(const config &filter) const
{
	...
	foreach_ng(const attribute &i, filter.attribute_range())
	{
		const attribute_value *v = get(i.first);
		if (!v || *v != i.second) return false;
	}

	foreach_ng(const any_child &i, filter.all_children_range())
	{
		if (i.key == "not") {
		...
		bool found = false;
		foreach_ng(const config &j, child_range(i.key)) {
			if (j.matches(i.cfg)) {
				found = true;
				break;
			}
...
std::ostream& operator << (std::ostream& outstream, const config& cfg)
{
	static int i = 0;
	i++;
	foreach_ng(const config::attribute &val, cfg.attribute_range()) {
		for (int j = 0; j < i-1; j++){ outstream << char(9); }
		outstream << val.first << " = " << val.second << '\n';
	}
	foreach_ng(const config::any_child &child, cfg.all_children_range())
	{
		for (int j = 0; j < i - 1; ++j) outstream << char(9);
		outstream << "[" << child.key << "]\n";
		outstream << child.cfg;
...
std::string config::hash() const
{
	...
	i = 0;
	foreach_ng(const attribute &val, values)
	{
	...
	foreach_ng(const any_child &ch, all_children_range())
	{
		std::string child_hash = ch.cfg.hash();
		foreach_ng(char c, child_hash) {
			hash_str[i] ^= c;
			++i;
			if(i == hash_length) {
				i = 0;
