...
struct dialog_process_info {
    public:
        ...
        CKey key;bool left_button, right_button, key_down, first_time, double_clicked;bool
            new_left_button, new_right_button, new_key_down;
        int selection;
    private:
        bool clear_buttons_;
};
...
class dialog_image: public widget {
    public:
        dialog_image(label * const caption, CVideo &video, SDL_Surface img) :
            widget(video, false), surf_(img), caption_(caption) {
            if ( !img.null()) {
...
class dialog_textbox: public textbox {
    public:
        dialog_textbox(label * const label_widget,
                       CVideo &video,
                       int width,
...
class dialog_button: public button {
    public:
        dialog_button(CVideo& video,
                      const std::string& label,
                      button_type_t type = TYPE_PRESS,
                      int simple_result = CONTINUE_DIALOG,
                      dialog_button_action *handler = nullptr) :
            button(video, label, type, "", DEFAULT_SPACE, false),
...
class standard_dialog_button: public dialog_button {
    public:
        standard_dialog_button(CVideo& video,
                               const std::string& label,
                               const int index,
                               const bool is_last) :
...
class dialog {
    ...
    public:
        ...
        void set_menu(menu * const m) {
            if (menu_ != empty_menu) delete menu_;
            menu_ = m == nullptr ? empty_menu : m;
        }
        void set_menu(const std::vector<std::string> & menu_items, menu::sorter* sorter =
            nullptr);
        void set_menu_items(const std::vector<std::string> &menu_items, bool keep_selection =
            false);

        //add_pane - preview panes are not currently memory managed
        //(for backwards compatibility)
        void add_pane(preview_pane * const pp) {
            preview_panes_.push_back(pp);
        }
        void set_panes(std::vector<preview_pane*> panes) {
            preview_panes_ = panes;
        }
        ...
        void add_option(const std::string& label, bool checked = false, BUTTON_LOCATION loc =
            BUTTON_CHECKBOX,
        ...
        bool done() const {
            return (result_ != CONTINUE_DIALOG);
        }
        std::string textbox_text() const {
            return text_widget_->text();
        }
