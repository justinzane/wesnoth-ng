...
bool filter_event(const event_handler& handler, const queued_event& ev) {
    ...
    BOOST_FOREACH(const vconfig &f, filters.get_children("filter_side")) {
        side_filter ssf(f);
        if ( !ssf.match(resources::controller->current_side())) return false;
    }
    ...
    if ( !special_matches && unit1 != units->end()) {
        ...
        BOOST_FOREACH(const vconfig &f, special_filters) {
            ...
            else if ( !matches_unit) return false;
...
bool process_event(event_handler& handler, const queued_event& ev) {
    ...
    if ( !filter_event(handler, ev)) return false;
...
void show_wml_messages(std::stringstream& source, const std::string & caption, bool to_cerr) {
    ...
    for (std::map<std::string, int>::const_iterator itor = messages.begin();
        ...
        if (to_cerr) std::cerr << caption << ": " << msg.str() << '\n';
    }
}
...
context::scoped::scoped() :
    ...
    new_context_(old_context_ != &context::default_context_ && old_context_->skip_messages) {
...
bool pump() {
    ...
    while ( !pump_instance.done()) {
        if (pump_manager::count() <= 1) manager::start_buffering();
        ...
        if (resources::lua_kernel->run_event(ev)) ++internal_wml_tracking;
        ...
        for (; cur_handler != end_handler; ++cur_handler) {
            ...
            if ( !handler.matches_name(event_name)) continue;
        ...
        if (pump_manager::count() <= 1) manager::stop_buffering();
