...
static TTF_Font* get_font(font_id id) {
    ...
    if (it != font_table.end()) return it->second;

    if (id.subset < 0 || size_t(id.subset) >= font_names.size()) return NULL;
...
static void clear_fonts() {
    for (std::map<font_id, TTF_Font*>::iterator i = font_table.begin(); i != font_table.end();
        ++i) {
...
const SDL_Color NORMAL_COLOR = { 0xDD, 0xDD, 0xDD, 0 }, GRAY_COLOR = { 0x77, 0x77, 0x77, 0 },
    LOBBY_COLOR = { 0xBB, 0xBB, 0xBB, 0 }, GOOD_COLOR = { 0x00, 0xFF, 0x00, 0 }, BAD_COLOR = {
        0xFF, 0x00, 0x00, 0 }, BLACK_COLOR = { 0x00, 0x00, 0x00, 0 },
    YELLOW_COLOR = { 0xFF, 0xFF, 0x00, 0 }, BUTTON_COLOR = { 0xBC, 0xB0, 0x88, 0 },
    PETRIFIED_COLOR = { 0xA0, 0xA0, 0xA0, 0 }, TITLE_COLOR = { 0xBC, 0xB0, 0x88, 0 },
    LABEL_COLOR = { 0x6B, 0x8C, 0xFF, 0 }, BIGMAP_COLOR = { 0xFF, 0xFF, 0xFF, 0 };
...
class text_surface {
    public:
        ...
        bool operator==(text_surface const &t) const {
            return hash_ == t.hash_ && font_size_ == t.font_size_ && color_ == t.color_
                   && style_ == t.style_ && str_ == t.str_;
        }
        bool operator!=(text_surface const &t) const {
            return !operator==(t);
        }
...
size_t text_surface::width() const {
    if (w_ == -1) {
        if (chunks_.empty()) chunks_ = split_text(str_);
        measure();
    }
    return w_;
}
...
std::vector<surface> const &text_surface::get_surfaces() const {
    ...
    if (width() > max_text_line_width) return surfs_;
...
static surface render_text(const std::string& text,
                           int fontsize,
    ...
    for (std::vector<std::string>::const_iterator ln = lines.begin(), ln_end = lines.end();
        ln != ln_end; ++ln) {
        ...
        std::string::const_iterator after_markup =
            use_markup ? parse_markup(ln->begin(), ln->end(), &sz, NULL, &text_style) :
                         ln->begin();
    ...
    if (surfaces.empty()) {
        ...
        size_t ypos = 0;
        for (std::vector<std::vector<surface> >::const_iterator i = surfaces.begin(), i_end =
            surfaces.end(); i != i_end; ++i) {
            size_t xpos = 0;
            size_t height = 0;

            for (std::vector<surface>::const_iterator j = i->begin(), j_end = i->end();
                j != j_end; ++j) {
                SDL_SetSurfaceAlphaMod( *j, 0);
...
SDL_Rect draw_text_line(surface gui_surface,
                        const SDL_Rect& area,
                        int size,
                        const SDL_Color& color,
...
std::string make_text_ellipsis(const std::string &text,
                               int font_size,
                               int max_width,
                               int style) {
    static const std::string ellipsis = "...";

    if (line_width(text, font_size, style) <= max_width) return text;
    if (line_width(ellipsis, font_size, style) > max_width) return "";
...
surface floating_label::create_surface() {
    if (surf_.null()) {
        ...
        if (bgalpha_ != 0) {
            ...
            surface background = create_neutral_surface(foreground->w + border_ * 2,
                                                        foreground->h + border_ * 2);
            ...
            Uint32 color = SDL_MapRGBA(foreground->format, bgcolor_.r, bgcolor_.g, bgcolor_.b,
                                       bgalpha_);
            ...
            SDL_SetSurfaceAlphaMod(foreground, SDL_SRCALPHA, SDL_ALPHA_OPAQUE);
            ...
            SDL_SetSurfaceAlphaMod(surf_, SDL_SRCALPHA, SDL_ALPHA_OPAQUE);
        } else {
...
static bool add_font_to_fontlist(config &fonts_config,
    ...
    for (std::vector<std::string>::const_iterator itor = ranges.begin(); itor != ranges.end();
        ++itor) {
...
bool load_font_config() {
    ...
    if (fontlist.empty()) return false;
