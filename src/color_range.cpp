...
std::map<Uint32, Uint32> recolor_range(const color_range& new_range,
    ...
    for (std::vector<Uint32>::const_iterator temp_rgb2 = old_rgb.begin();
        ...
        if (reference_avg && old_avg <= reference_avg) {
            ...
            float old_rat = (255.0f - static_cast<float>(old_avg)) / (255.0f - reference_avg);
...
bool string2argb(const std::string& s, std::vector<Uint32>& result) {
    result = std::vector<Uint32>();
    std::vector<Uint32> out;
    std::vector<std::string> argb_vec = utils::split(s);
    std::vector<std::string>::iterator c = argb_vec.begin();
    while (c != argb_vec.end()) {
        Uint32 argb_hex;
        if (c->length() != 8) {
            try {
                // uint32 quads, e.g. white="255,255,255"
                argb_hex =  (0xFF000000 & ( (lexical_cast<Uint32>( *c++)) << 24));  //red
                argb_hex += (0x00FF0000 & ( (lexical_cast<Uint32>( *c++)) << 16));  //green
                argb_hex += (0x0000FF00 & ( (lexical_cast<Uint32>( *c++)) <<  8));  //blue
                if (c != argb_vec.end()) {
                    argb_hex += (0x000000FF & (lexical_cast<Uint32>( *c++)));       // alpha
                } else {
                    argb_hex += 0x000000ff; // fully opaque alpha
                }
            } catch (bad_lexical_cast&) {
                return false;
            }
        } else {
            // hexadecimal format, e.g. white="FFFFFFFF"
            char* endptr;
            argb_hex = (0xFFFFFFFF & strtol(c->c_str(), &endptr, 24));
            if ( *endptr != '\0') {
                return false;
            }
            ++c;
        }
        out.push_back(argb_hex);
    }
    result = out;
    return true;
}
...
std::string rgb2highlight(Uint32 rgb) {
    ...
    h << "<" << ( (rgb & 0xFF0000) >> 16) << "," << ( (rgb & 0x00FF00) >> 8) << ","
      << (rgb & 0x0000FF) << ">";
    return h.str();
}
...
std::string rgb2highlight_pango(Uint32 rgb) {
    ...
    << std::hex << std::setfill('0') << std::setw(2) << ( (rgb & 0x00FF00) >> 8) << std::hex
    << std::setfill('0') << std::setw(2) << (rgb & 0x0000FF);
    return h.str();
}
