...
game_controller::game_controller(const commandline_options& cmdline_opts, const char *appname) :
...
#ifdef _WIN32
        // use c_str to ensure that index 1 points to valid element since c_str()
    ...
    if (cmdline_opts_.clock) gui2::show_debug_clock_button = true;
    ...
    if (cmdline_opts_.fps) preferences::set_show_fps(true);
    if (cmdline_opts_.fullscreen) preferences::set_fullscreen(true);
    if (cmdline_opts_.load) game::load_game_exception::game = *cmdline_opts_.load;
    ...
    if (cmdline_opts_.new_widgets) gui2::new_widgets = true;
    if (cmdline_opts_.nodelay) game_config::no_delay = true;
    if (cmdline_opts_.nomusic) no_music = true;
    if (cmdline_opts_.nosound) no_sound = true;
    ...
    if (cmdline_opts_.proxy) network::enable_connection_through_proxy();
    ...
    if (cmdline_opts_.test) {
        if ( !cmdline_opts_.test->empty()) test_scenario_ = *cmdline_opts_.test;
    }
    if (cmdline_opts_.windowed) preferences::set_fullscreen(false);
    if (cmdline_opts_.with_replay) game::load_game_exception::show_replay = true;
...
bool game_controller::init_joystick() {
    if ( !preferences::joystick_support_enabled()) return false;

    if (SDL_WasInit(SDL_INIT_JOYSTICK) == 0)
        if (SDL_InitSubSystem(SDL_INIT_JOYSTICK) == -1) return false;
    ...
    for (int i = 0; i < joysticks; i++) {
        ...
        if (joystick) joystick_found = true;
    }
    return joystick_found;
}

bool game_controller::init_language() {
    if ( !::load_language_list()) return false;
    ...
    if ( !cmdline_opts_.nogui) {
        ...
        SDL_SetWindowTitle(wes_win_, wm_title_string.c_str());
    }
...
bool game_controller::init_video() {
    if (cmdline_opts_.nogui) {
        if ( ! (cmdline_opts_.multiplayer || cmdline_opts_.screenshot)) {
            std::cerr
                << "--nogui flag is only valid with --multiplayer flag or --screenshot flag\n";
            return false;
        }
        video_.make_fake();
    ...
    if (icon != NULL) {
        ...
        SDL_SetWindowIcon(wes_win_, icon);
    }
#endif
    ...
    bool found_matching = preferences::detect_video_settings(video_, resolution, bpp,
                                                             video_flags);
    ...
    if ( !found_matching) {
        std::cerr << "Video mode " << resolution.first << 'x' << resolution.second << 'x' << bpp
                  << " is not supported.\n";

        if ( (video_flags & FULL_SCREEN)) {
            std::cerr << "Try running the program with the --windowed option " << "using a "
                      << bpp << "bpp setting for your display adapter.\n";
        } else {
    ...
    std::cerr << "setting mode to " << resolution.first << "x" << resolution.second << "x"
              << bpp << "\n";
    const int res = video_.setMode(resolution.first, resolution.second, bpp, video_flags);
    video_.setBpp(bpp);
    if (res == 0) {
        std::cerr << "required video mode, " << resolution.first << "x" << resolution.second
                  << "x" << bpp << " is not supported\n";
        return false;
    }
...
bool game_controller::play_screenshot_mode() {
    ...
    editor::start(resources::config_manager->game_config(), video_, screenshot_map_, true,
                  screenshot_filename_);
    return false;
}
...
bool game_controller::load_game() {
    savegame::loadgame load(disp(), resources::config_manager->game_config(), state_);

    try {
        load.load_game(game::load_game_exception::game, game::load_game_exception::show_replay,
                       game::load_game_exception::cancel_orders,
                       game::load_game_exception::select_difficulty,
                       game::load_game_exception::difficulty);
        ...
        if (e.message.empty()) {
            ...
            gui2::show_error_message(
                disp().video(),
                _("The file you have tried to load is corrupt: '") + e.message + '\'');
        }
        return false;
        ...
        if (e.message.empty()) {
            gui2::show_error_message(disp().video(),
                                     _("File I/O Error while reading the game"));
        } else {
            gui2::show_error_message(
                disp().video(),
                _("File I/O Error while reading the game: '") + e.message + '\'');
        }
        return false;
    } catch (game::error& e) {
        if (e.message.empty()) {
    ...
    if (state_.classification().campaign_type == "multiplayer") {
        BOOST_FOREACH(config &side, state_.snapshot.child_range("side")) {
            if (side["controller"] == "network") side["controller"] = "human";
            if (side["controller"] == "network_ai") side["controller"] = "human_ai";
        }
    }
...
bool game_controller::new_campaign() {
    ...
    if (difficulties.empty() == false) {
        ...
        if (jump_to_campaign_.difficulty_ == -1) {
            ...
            if (jump_to_campaign_.difficulty_ > static_cast<int>(difficulties.size())) {
                std::cerr << "incorrect difficulty number: [" << jump_to_campaign_.difficulty_
...
bool game_controller::goto_editor() {
    if (jump_to_editor_) {
        jump_to_editor_ = false;
        if (start_editor(normalize_path(game::load_game_exception::game)) == editor::EXIT_QUIT_TO_DESKTOP) {
            return false;
        }
        clear_loaded_game();
    }
...
bool game_controller::play_multiplayer() {
    ...
    try {
        ...
        if (res == 3) {
            mp::start_local_game(disp(), resources::config_manager->game_config(), state_);
            ...
            mp::start_client(disp(), resources::config_manager->game_config(), state_, host);
        }

    } catch (game::mp_server_error& e) {
        gui2::show_error_message(disp().video(),
                                 _("Error while starting server: ") + e.message);
    } catch (game::load_game_failed& e) {
        gui2::show_error_message(disp().video(),
                                 _("The game could not be loaded: ") + e.message);
    } catch (game::game_error& e) {
        gui2::show_error_message(disp().video(),
                                 _("Error while playing the game: ") + e.message);
...
bool game_controller::play_multiplayer_commandline() {
    ...
    mp::start_local_game_commandline(disp(), resources::config_manager->game_config(), state_,
                                     cmdline_opts_);
...
void game_controller::show_preferences() {
    ...
    preferences::show_preferences_dialog(disp(), resources::config_manager->game_config());
...
void game_controller::launch_game(RELOAD_GAME_DATA reload) {
    ...
    try {
        ...
        if (result == VICTORY && (state_.classification().campaign_type.empty()
            || state_.classification().campaign_type != "multiplayer")) {
            preferences::add_completed_campaign(state_.classification().campaign);
            the_end(disp(), state_.classification().end_text,
...
void game_controller::play_replay() {
    try {
        ::play_replay(disp(), state_, resources::config_manager->game_config(), video_);
...
editor::EXIT_STATUS game_controller::start_editor(const std::string& filename) {
    while (true) {
        ...
        editor::EXIT_STATUS res = editor::start(resources::config_manager->game_config(),
                                                video_, filename);

        if (res != editor::EXIT_RELOAD_DATA) return res;
