...
void loadscreen::draw_screen(const std::string &text) {
    ...
    // Groove.
    int lca = 191, lcr = 21, lcg = 22, lcb = 24;
    // Border color.
    int bca = 191, bcr = 188, bcg = 176, bcb = 136;
    ...
    area.x = pbx;
    area.y = pby;
    area.w = pbw + 2 * (bw + bispw);
    area.h = bw;
    SDL_FillRect(gdis, &area, SDL_MapRGBA(gdis->format, bcr, bcg, bcb, bca));
    // Draw bottom border.
    area.x = pbx;
    area.y = pby + pbh + bw + 2 * bispw;
    area.w = pbw + 2 * (bw + bispw);
    area.h = bw;
    SDL_FillRect(gdis, &area, SDL_MapRGBA(gdis->format, bcr, bcg, bcb, bca));
    // Draw left border.
    ...
    area.x = pbx + bw + bispw;
    area.y = pby + bw + bispw;
    area.w = (prcnt_ * pbw) / 100;
    area.h = pbh;
    SDL_FillRect(gdis, &area, SDL_MapRGBA(gdis->format, fcr, fcg, fcb, fca));
    ...
    SDL_FillRect(gdis, &lightning, SDL_MapRGBA(gdis->format, fcr / 2, fcg / 2, fcb / 2, fca));

    // Draw the leftover bar area.
    area.x = pbx + bw + bispw + (prcnt_ * pbw) / 100;
    area.y = pby + bw + bispw;
    area.w = ( (100 - prcnt_) * pbw) / 100;
    area.h = pbh;
    SDL_FillRect(gdis, &area, SDL_MapRGBA(gdis->format, lcr, lcg, lcb, lca));

    // Clear the last text and draw new if text is provided.
    if ( !text.empty()) {
        SDL_Rect oldarea = textarea_;
        SDL_FillRect(gdis, &textarea_, SDL_MapRGBA(gdis->format, 0, 0, 0, 255));
        textarea_ = font::line_size(text, font::SIZE_NORMAL);
        textarea_.x = scrx / 2 + bw + bispw - textarea_.w / 2;
        textarea_.y = pby + pbh + 4 * (bw + bispw);
        textarea_ = font::draw_text( &screen_, textarea_, font::SIZE_NORMAL, font::NORMAL_COLOR,
                                    text, textarea_.x, textarea_.y);
...
void loadscreen::clear_screen() {
    ...
    SDL_Surface disp(screen_.getSurface());      // Screen surface.
    // Make everything black.
    SDL_PixelFormat fmt = SDL_PIXELFORMAT_ARGB8888;
    SDL_FillRect(disp, &area, SDL_MapRGBA(&fmt, 0, 0, 0, 255));
    update_whole_screen();
    screen_.flip();
}
...
static load_stage const stages[nb_stages] = {
    { "init gui", N_("Initializing user interface"), 0, 17210 }, { "load config", N_(
        "Loading game configuration"),
                                                                   20, 0 },
    { "verify cache", N_("Verifying cache"), 20, 164 }, { "create cache", N_(
        "Reading files and creating cache"),
                                                          21, 80287 },
    { "load unit types", N_("Reading unit files"), 94, 467 }, { "init fonts", N_(
        "Reinitialize fonts for the current language"),
                                                                98, 17 },
    { "refresh addons", N_("Searching for installed add-ons"), 99, 0 }, { "titlescreen", N_(
        "Loading title screen"),
                                                                          100, 0 },
    { "load data", N_("Loading data files"), 0, 0 }, { "verify cache", N_("Verifying cache"), 0,
                                                       0 },
    { "create cache", N_("Reading files and creating cache"), 0, 152109 }, {
        "load unit types", N_("Reading unit files"), 74, 488 },
    { "load level", N_("Loading level"), 75, 0 }, { "init teams", N_("Initializing teams"), 75,
                                                    0 },
    { "load units", N_("Loading units"), 77, 0 }, { "init theme", N_("Initializing display"),
                                                    77, 0 },
    { "build terrain", N_("Building terrain rules"), 77, 1240 }, { "init display", N_(
...
void loadscreen::dump_counters() const {
    ...
    while (i < nb_stages) {
        ...
        for (int k = i; k <= j; ++k) {
            ...
            if (i < k && k < j) {
                v = stages[i].start_pos + (100 - stages[i].start_pos)
                    * (stage_time[k] - stage_time[i])
                                          / (stage_time[j] - stage_time[i]);
            }
            s << "\t{ \"" << stages[k].id << "\", N_(\"" << stages[k].name << "\"), " << v
              << ", " << stage_counter[k] << " },\n";
        }
        i = j + 1;
    }
