 ...
 * @brief 
 * @authors 
 * @copyright Copyright (C) 2010 - 2013 by Gabriel Morin <gabrielmorin (at) gmail (dot) com>
...
bool mgr::allow_leader_to_move(unit const& leader) const
{
	...
	foreach_ng(action_const_ptr action, *viewer_actions())
	{
...
void mgr::process_network_data(config const& cfg)
{
	if(config const& wb_cfg = cfg.child("whiteboard"))
	{
		...
		foreach_ng(side_actions::net_cmd const& cmd, wb_cfg.child_range("net_cmd"))
