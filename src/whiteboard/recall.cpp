...
std::ostream& recall::print(std::ostream &s) const {
    s << "Recalling " << fake_unit_->name() << " [" << fake_unit_->id() << "] on hex "
    << recall_hex_;
    return s;
}
...
recall::recall(config const& cfg, bool hidden) :
    ...
    std::string underlying_id = cfg["temp_unit_"];
...
void recall::execute(bool& success, bool& complete) {
    ...
    bool const result = actions::recall_unit(temp_unit_->id(), current_team, recall_hex_,
...
void recall::remove_temp_modifier(unit_map& unit_map) {
    ...
    resources::teams->at(team_index()).recall_list().push_back(temp_unit_.operator *());
}
 ...
 *  @note Changed from int id to uuid-as-string id. Planning on eliminating underlying id
 *  completely. */
config recall::to_config() const {
    config final_cfg = action::to_config();

    final_cfg["type"] = "recall";
    final_cfg["temp_unit_"] = temp_unit_->underlying_id();
