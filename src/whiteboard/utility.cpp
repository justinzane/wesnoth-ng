...
side_actions_ptr viewer_actions() {
    side_actions_ptr side_actions = ( *resources::teams)[resources::screen->viewing_team()]
        .get_side_actions();
    return side_actions;
}

side_actions_ptr current_side_actions() {
    side_actions_ptr side_actions = ( *resources::teams)[resources::controller->current_side()
        - 1].get_side_actions();
    return side_actions;
}
...
unit const* find_backup_leader(unit const& leader) {
    ...
    BOOST_FOREACH(unit const& unit, *resources::units) {
        if (unit.can_recruit() && unit.id() != leader.id()) {
            if (can_recruit_on(unit, leader.get_location())) return &unit;
        }
    }
    return NULL;
}
...
unit* find_recruiter(size_t team_index, map_location const& hex) {
    if ( !resources::game_map->is_castle(hex)) return NULL;

    BOOST_FOREACH(unit& u, *resources::units)
        if (u.can_recruit() && u.side() == static_cast<int>(team_index + 1)
            && can_recruit_on(u, hex)) return &u;
    return NULL;
}
...
int path_cost(std::vector<map_location> const& path, unit const& u) {
    ...
    if ( (resources::game_map->is_village(dest) && !u_team.owns_village(dest)) || pathfind::enemy_zoc(
        u_team, dest, u_team)) return u.total_movement();
...
bool has_actions() {
    BOOST_FOREACH(team& t, *resources::teams)
        if ( !t.get_side_actions()->empty()) return true;
