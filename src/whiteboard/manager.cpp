...
bool manager::can_modify_game_state() const {
    if (wait_for_side_init_ || resources::teams == NULL || executing_actions_ || is_observer()
...
bool manager::can_activate() const {
    ...
    if ( !activation_state_lock_.unique()) return false;
...
void manager::set_invert_behavior(bool invert) {
    //any more than one reference means a lock on whiteboard state was requested
    if ( !activation_state_lock_.unique()) return;
...
bool manager::can_enable_execution_hotkeys() const {
    return can_enable_modifier_hotkeys()
        && viewer_side() == resources::controller->current_side()
...
bool manager::allow_leader_to_move(unit const& leader) const {
    ...
    //Look for another leader on another keep in the same castle
    {
        ...
        if (find_backup_leader(leader)) return true;
    ...
    BOOST_FOREACH(action_const_ptr action, *viewer_actions()) {
        ...
        if (recruit || recall) {
            ...
            if (can_recruit_on(leader, target_hex)) return false;
        }
    }
    return true;
}
...
void manager::post_delete_action(action_ptr action) {
    ...
    side_actions_ptr side_actions =
        resources::teams->at(action->team_index()).get_side_actions();
...
void manager::update_plan_hiding(size_t team_index) {
    ...
    else  //< normal circumstance
    {
        BOOST_FOREACH(team& t, *resources::teams) {
            ...
            if ( !t.is_network_human()) team_plans_hidden_[t.side() - 1] = false;
...
void manager::on_viewer_change(size_t team_index) {
    if ( !wait_for_side_init_) update_plan_hiding(team_index);
}
    ...
    else if (t.is_network())  //< Another client is taking control of the side
    {
        ...
        for (size_t i = 0; i < num_teams; ++i) {
            ...
            if (local_team.is_human() && !local_team.is_enemy(side))
                resources::whiteboard->queue_net_cmd(
                    i, local_team.get_side_actions()->make_net_cmd_refresh());
        }
    }
}
...
static void draw_numbers(map_location const& hex, side_actions::numbers_t numbers) {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        resources::screen->draw_text_in_hex(hex, display::LAYER_ACTIONS_NUMBERING, number_text,
                                            font_size, color, x_in_hex, y_in_hex);
        x_offset += x_offset_base;
        y_offset += y_offset_base;
    }
}
/** @namespace
 * @brief Helper struct that finds all units teams whose planned actions are currently
 * visible Only used by manager::pre_draw().
...
struct move_owners_finder: public visitor {
    public:
        ...
        std::set<std::string> const& get_units_owning_moves() {
            return move_owners_;
        }

        virtual void visit(move_ptr move) {
            std::string id = move->get_unit_id();
            if (id != "") {
        ...
        virtual void visit(attack_ptr attack) {
            ...
            if (attack->get_route().steps.size() >= 2) {
                std::string id = attack->get_unit_id();
                if (id != "") {
                    move_owners_.insert(id);
                }
            }
    ...
    private:
        std::set<std::string> move_owners_;
};
...
void manager::pre_draw() {
    if (can_modify_game_state() && has_actions()) {
        ...
        BOOST_FOREACH(std::string unit_id, units_owning_moves_) {
...
void manager::draw_hex(const map_location& hex) {
    ...
    if ( !wait_for_side_init_ && has_actions()) {
        ...
        BOOST_FOREACH(team& t, *resources::teams) {
            ...
            if ( !sa.hidden()) sa.get_numbers(hex, numbers);
        }
...
void manager::on_mouseover_change(const map_location& hex) {
    ...
    map_location selected_hex =
        resources::controller->get_mouse_handler_base().get_selected_hex();
    bool hex_has_unit;
    {
        wb::future_map future;  //< start planned unit map scope
        hex_has_unit = resources::units->find(selected_hex) != resources::units->end();
    }  // end planned unit map scope
    if ( ! ( (selected_hex.valid() && hex_has_unit) || has_temp_move() || wait_for_side_init_
            || executing_actions_)) {
        if ( !highlighter_) {
...
void manager::create_temp_move() {
    ...
    if ( !active_ || !can_modify_game_state()) return;

    pathfind::marked_route const& route = resources::controller->get_mouse_handler_base()
        .get_current_route();

    if (route.steps.empty() || route.steps.size() < 2) return;

    unit* temp_moved_unit = future_visible_unit(
        resources::controller->get_mouse_handler_base().get_selected_hex(), viewer_side());
    if ( !temp_moved_unit)
        temp_moved_unit = future_visible_unit(
            resources::controller->get_mouse_handler_base().get_last_hex(), viewer_side());
    ...
    for (; curr_itor != end_itor; ++curr_itor) {
        ...
        if (w != route.marks.end() && w->second.turns > 0) {
            ...
            if (turn >= move_arrows_.size()) move_arrows_.resize(turn + 1);
            if (turn >= fake_units_.size()) fake_units_.resize(turn + 1);
            ...
            if ( !move_arrow) {
                ...
                move_arrow->set_color(team::get_side_color_index(viewer_side()));
            ...
            if (path.size() >= 2) {
                ...
                //zero-hex path -- don't bother drawing a fake unit
                fake_unit.reset();
...
void manager::save_temp_move() {
    if (has_temp_move() && !executing_actions_ && !resources::controller->is_linger_mode()) {
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            if ( !arrow::valid_path(move_arrow->get_path())) continue;
...
void manager::save_temp_attack(const map_location& attacker_loc,
    ...
    if (active_ && !executing_actions_ && !resources::controller->is_linger_mode()) {
        ...
        side_actions& sa = *viewer_actions();
        sa.queue_attack(sa.get_turn_num_of( *attacking_unit), *attacking_unit, defender_loc,
                        weapon_choice, *route_, move_arrow, fake_unit);
...
bool manager::save_recruit(const std::string& name,
                           int side_num,
...
void manager::contextual_execute() {
    ...
    if (can_enable_execution_hotkeys()) {
        ...
        unit const* selected_unit = future_visible_unit(
            resources::controller->get_mouse_handler_base().get_selected_hex(), viewer_side());
        if (selected_unit && (it = viewer_actions()->find_first_action_of( *selected_unit))
            != viewer_actions()->end()) {
            executing_actions_ = true;
            viewer_actions()->execute(it);
        } else if (highlighter_
            && (action = highlighter_->get_execute_target())
...
void manager::contextual_delete() {
    ...
    if (can_enable_modifier_hotkeys()) {
        ...
        if (selected_unit && (it = viewer_actions()->find_first_action_of( *selected_unit))
            ...
            && (action = highlighter_->get_delete_target())
            && (it = viewer_actions()->get_position_of(action)) != viewer_actions()->end()) {
            viewer_actions()->remove_action(it);
            validate_viewer_actions();
...
int manager::get_spent_gold_for(int side) {
    if (wait_for_side_init_) return 0;
...
void manager::options_dlg() {
    ...
    BOOST_FOREACH(team &t, *resources::teams) {
        ...
        if (t.is_enemy(v_side) || !t.is_network()) continue;
...
future_map::future_map() :
    initial_planned_unit_map_(
        resources::whiteboard && resources::whiteboard->has_planned_unit_map()) {
    if ( !resources::whiteboard) return;
    if ( !initial_planned_unit_map_) resources::whiteboard->set_planned_unit_map();
...
real_map::real_map() :
    initial_planned_unit_map_(
        resources::whiteboard && resources::whiteboard->has_planned_unit_map()),
    unit_map_lock_(
        resources::whiteboard ? resources::whiteboard->unit_map_lock_ :
                                boost::shared_ptr<bool>(new bool(false))) {
    if ( !resources::whiteboard) return;
    if (initial_planned_unit_map_) resources::whiteboard->set_real_unit_map();
}
