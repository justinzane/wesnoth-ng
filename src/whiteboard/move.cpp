...
std::ostream& move::print(std::ostream &s) const {
    s << "Move for unit " << get_unit()->name() << " [" << get_unit()->id() << "] " << "from ("
    << get_source_hex() << ") to (" << get_dest_hex() << ")";
    return s;
}
...
move::move(size_t team_index,
    ...
    if (hidden) fake_unit_->set_hidden(true);
...
move::move(config const& cfg, bool hidden) :
    ...
    if ( !route_cfg) throw action::ctor_err("move: Invalid route_");
    ...
    BOOST_FOREACH(config const& mark_cfg, route_cfg.child_range("mark")) {
        route_->marks[map_location(mark_cfg["x"], mark_cfg["y"])] =
            pathfind::marked_route::mark(mark_cfg["turns"], mark_cfg["zoc"],
                                         mark_cfg["capture"], mark_cfg["invisible"]);
    }

    // Validate route_ some more
    std::vector<map_location> const& steps = route_->steps;
    if (steps.empty()) throw action::ctor_err("move: Invalid route_");
...
void move::init() {
    ...
    if (action != side_actions->end()) {
        if (move_ptr move = boost::dynamic_pointer_cast<class move>( *action)) {
            if (move->fake_unit_) move->fake_unit_->set_disabled_ghosted(true);
        }
    }
...
bool move::calculate_new_route(const map_location& source_hex, const map_location& dest_hex) {
    ...
    new_plain_route = pathfind::a_star_search(source_hex, dest_hex, 10000, &path_calc,
                                              resources::game_map->w(),
                                              resources::game_map->h());
...
void move::apply_temp_modifier(unit_map& unit_map) {
    if (get_source_hex() == get_dest_hex()) return;  //zero-hex move, used by attack subclass
    ...
    mover_.reset(
        new temporary_unit_mover(unit_map, get_source_hex(), get_dest_hex(),
                                 unit->movement_left() - movement_cost_));
...
void move::remove_temp_modifier(unit_map&) {
    if (get_source_hex() == get_dest_hex()) return;  //zero-hex move, probably used by attack subclass
...
void move::draw_hex(map_location const& hex) {
    ...
    if (hex == get_dest_hex() && turn_number_ >= 2) {
        ...
        resources::screen->draw_text_in_hex(hex, display::LAYER_MOVE_INFO, turn_text.str(), 17,
                                            font::NORMAL_COLOR, 0.5, 0.8);
    }
}

void move::do_hide() {
    arrow_->hide();
    if ( !fake_unit_hidden_) fake_unit_->set_hidden(true);
}

void move::do_show() {
    arrow_->show();
    if ( !fake_unit_hidden_) fake_unit_->set_hidden(false);
}
...
void move::show_fake_unit() {
    fake_unit_hidden_ = false;
    if ( !hidden()) fake_unit_->set_hidden(false);
}
...
action::error move::check_validity() const {
    ...
    class arrow_texture_setter {
        public:
            arrow_texture_setter(const move *target,
                                 move::ARROW_TEXTURE current_texture,
                                 move::ARROW_TEXTURE setting_texture) :
                target(target),
    ...
    if (get_route().steps.size() >= 2 && get_visible_unit(get_dest_hex(),
                                                          resources::teams->at(viewer_team()))
                                         != NULL) {
...
config move::to_config() const {
    ...
    final_cfg["type"] = "move";
    final_cfg["unit_"] = unit_underlying_id_;
...
void move::calculate_move_cost() {
    ...
    if (get_source_hex().valid() && get_dest_hex().valid()
        ...
        if (get_unit()->movement_left() - route_->move_cost < 0 && resources::controller
            ->current_side()
                                                                   == resources::screen
                                                                       ->viewing_side()) {
