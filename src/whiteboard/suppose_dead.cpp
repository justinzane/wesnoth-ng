...
suppose_dead::suppose_dead(size_t team_index,
                           bool hidden,
                           unit& curr_unit,
...
suppose_dead::~suppose_dead() {
    ...
    if (resources::screen) resources::screen->invalidate(loc_);
}
...
void suppose_dead::draw_hex(const map_location& hex) {
    if (hex == loc_)  //add symbol to hex
    {
        ...
        resources::screen->drawing_buffer_add(
            layer, loc_, xpos, ypos,
