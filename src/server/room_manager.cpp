 ...
 * @brief 
 * @authors 
 * @copyright Copyright (C) 2009 - 2013 by Tomasz Sniatowski <kailoran@gmail.com>
...
room_manager::~room_manager()
{
	...
	write_rooms();
	foreach_ng(t_rooms_by_name_::value_type i, rooms_by_name_) {
		delete i.second;
	}
}
...
void room_manager::read_rooms()
{
	if (!filename_.empty() && file_exists(filename_)) {
		...
		foreach_ng(const config &c, cfg.child_range("room")) {
			room* r(new room(c));
...
void room_manager::write_rooms()
{
	...
	config cfg;
	foreach_ng(const t_rooms_by_name_::value_type& v, rooms_by_name_) {
...
void room_manager::unstore_player_rooms(const player_map::iterator user)
{
	...
	foreach_ng(const std::string& room_name, it->second) {
