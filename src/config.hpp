...
class config {
        friend bool operator==(const config& a,
                               const config& b);
    ...
    public:
        ...
        class attribute_value {
                ...
                class true_false {
                    ...
                    public:
                        ...
                        const std::string & str() const {
                            return
                                value_ ? config::attribute_value::s_true :
                ...
                class yes_no {
                    ...
                    public:
                        ...
                        const std::string & str() const {
                            return
                                value_ ? config::attribute_value::s_yes :
        ...
        typedef std::pair<const_attribute_iterator,
                          const_attribute_iterator> const_attr_itors;
         ...
         *       For instance, -1 is the index of the last child.
         */
        const config &child(const std::string& key,
                            int n = 0) const {
        ...
        config& add_child(const std::string& key,
                          const config& val);
        config& add_child_at(const std::string &key,
                             const config &val,
                             unsigned index);
         ...
         * and log msg as a WML error (if not empty)
         */
        bool has_old_attribute(const std::string &key,
                               const std::string &old_key,
                               const std::string& msg = "") const;
         ...
         * containing @a value.
         */
        config &find_child(const std::string &key,
                           const std::string &name,
                           const std::string &value);

        const config &find_child(const std::string &key,
                                 const std::string &name,
    ...
    private:
         ...
         */
        std::vector<child_pos>::iterator remove_child(const child_map::iterator &l,
                                                      unsigned pos);
...
inline std::ostream &operator<<(std::ostream &os, const config::attribute_value::yes_no &v) {
    return os << v.str();
}
