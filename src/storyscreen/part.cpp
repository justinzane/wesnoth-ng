...
floating_image::render_input floating_image::get_render_input(double xscale,
                                                              double yscale,
                                                              SDL_Rect* dst_rect) const {
    render_input ri = { { 0, 0, 0, 0 }, file_.empty() ? nullptr : image::get_image(file_) };

    if ( !ri.image.null()) {
        if (autoscaled_) {
            ri.image = scale_surface(ri.image, static_cast<int>(ri.image->w * xscale),
    ...
    for (vconfig::all_children_iterator i = cfg.ordered_begin(); i != cfg.ordered_end(); ++i) {
        ...
        // [if]
            ...
            const std::string branch_label =
                game_events::conditional_passed(node) ? "then" : "else";
        ...
        // [switch]
            ...
            const std::string var_actual_value = resources::gamedata->get_variable_const(
                var_name);
            bool case_not_found = true;

            for (vconfig::all_children_iterator j = node.ordered_begin();
                j != node.ordered_end(); ++j) {
