	...
	, links_()
{
		...
		foreach_ng(const config &child, cfg.child_range("tag")) {
			class_tag child_tag (child);
			add_tag(child_tag);
		}
		foreach_ng(const config &child, cfg.child_range("key")) {
			class_key child_key (child);
			add_key(child_key);
		}
		foreach_ng(const config &link, cfg.child_range("link")) {
