...
void surf2rgba_i(SDL_Surface* surf, Uint32* red, Uint32* green, Uint32* blue, Uint32* alpha) {
    ...
    if (surf->format->BitsPerPixel != 32) {     // Make sure we have a 32bpp surface.
        printf("JZ\t surface not 32bpp.!\n");
        return;
    }
    ...
    if (surf->format->Aloss == 8) {     // Aloss==8 means RGB not RGBA
        for (y = 0; y < surf->h; y++) {
            for (x = 0; x < surf->w; x++) {
                p = surf->w * y + x;
                red[p] =   ((sdl_pix[p] & surf->format->Rmask) >> surf->format->Rshift);
                green[p] = ((sdl_pix[p] & surf->format->Gmask) >> surf->format->Gshift);
                blue[p] =  ((sdl_pix[p] & surf->format->Bmask) >> surf->format->Bshift);
                alpha[p] = 255;
            }
        }
    } else {
        for (y = 0; y < surf->h; y++) {
            for (x = 0; x < surf->w; x++) {
                ...
                green[p] = ((sdl_pix[p] & surf->format->Gmask) >> surf->format->Gshift);
                blue[p] =  ((sdl_pix[p] & surf->format->Bmask) >> surf->format->Bshift);
                alpha[p] = ((sdl_pix[p] & surf->format->Amask) >> surf->format->Ashift);
            }
        }
    }
}
