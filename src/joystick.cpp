...
joystick_manager::joystick_manager() :
    joysticks_(), joystick_area_(0), counter_(0) {
    init();
}
...
bool joystick_manager::close() {
    if (SDL_WasInit(SDL_INIT_JOYSTICK) == 0) return true;
...
std::pair<double, double> joystick_manager::get_mouse_axis_pair() {
    ...
    if (deadzone > radius) return std::make_pair(0.0, 0.0);
    ...
    return std::make_pair( ( (static_cast<double>(values.first)) / 32768.0) * multiplier,
...
std::pair<double, double> joystick_manager::get_cursor_polar_coordinates() {
    ...
    return get_polar_coordinates(cursor_joystick_xaxis, cursor_xaxis, cursor_joystick_yaxis,
                                 cursor_yaxis);
}
...
std::pair<double, double> joystick_manager::get_polar_coordinates(int joystick_xaxis,
    ...
    const std::pair<int, int> values = get_axis_pair(joystick_xaxis, xaxis, joystick_yaxis,
                                                     yaxis);
    const double radius = (sqrt(pow(values.first, 2.0f) + pow(values.second, 2.0f))) / 32768.0;
    const double angle = (atan2(static_cast<double>(values.second),
                                static_cast<double>(values.first)))
                         * 180.0
                         / pi<double>();
...
std::pair<int, int> joystick_manager::get_axis_pair(int joystick_xaxis,
    ...
    if ( !SDL_WasInit(SDL_INIT_JOYSTICK)) return std::make_pair(0, 0);

    int x_axis = 0, y_axis = 0;
    bool get_xaxis = false, get_yaxis = false;

    if (attached(joysticks_, joystick_xaxis))
        if (SDL_JoystickNumAxes(joysticks_[joystick_xaxis]) > xaxis) get_xaxis = true;

    if (attached(joysticks_, joystick_yaxis))
        if (SDL_JoystickNumAxes(joysticks_[joystick_yaxis]) > yaxis) get_yaxis = true;
...
int joystick_manager::get_axis(int joystick_axis, int axis) {
    if ( !SDL_WasInit(SDL_INIT_JOYSTICK)) return 0;
...
bool joystick_manager::update_highlighted_hex(map_location& highlighted_hex,
    ...
    const std::pair<int, int> values = get_axis_pair(cursor_joystick_xaxis, cursor_xaxis,
                                                     cursor_joystick_yaxis, cursor_yaxis);
...
const map_location joystick_manager::get_direction(const map_location& loc,
    ...
    switch (direction) {
        case NORTH:
            return map_location(x, y - 1);
        case SOUTH:
            return map_location(x, y + 1);
        case SOUTH_EAST:
            return map_location(x + 1, y + (1 + is_odd(x)) / 2);
        case SOUTH_WEST:
            return map_location(x - 1, y + (1 + is_odd(x)) / 2);
        case NORTH_EAST:
            return map_location(x + 1, y - (1 + is_even(x)) / 2);
        case NORTH_WEST:
            return map_location(x - 1, y - (1 + is_even(x)) / 2);
        case WEST:
            return map_location(x - 1, y);
        case EAST:
            return map_location(x + 1, y);
        default:
            assert(false);
...
double joystick_manager::get_angle() {
    ...
    const double angle = (atan2(static_cast<double>(y_axis), static_cast<double>(x_axis)))
        * 180.0
                         / pi<double>();
...
const map_location joystick_manager::get_next_hex(int x_axis, int y_axis, map_location loc) {
    ...
    if (x_axis == 0)
        return (y_axis > 0) ? get_direction(loc, SOUTH) : get_direction(loc, NORTH);
    if (y_axis == 0) return (x_axis > 0) ? get_direction(loc, EAST) : get_direction(loc, WEST);
    const double angle = (atan2(static_cast<double>(y_axis), static_cast<double>(x_axis)))
        * 180.0
                         / pi<double>();

    if (angle < -112.5 && angle > -157.5) new_loc = get_direction(loc, NORTH_WEST);

    if (angle < -67.5 && angle > -112.5) new_loc = get_direction(loc, NORTH);

    if (angle < -22.5 && angle > -67.5) new_loc = get_direction(loc, NORTH_EAST);

    if (angle < 22.5 && angle > -22.5) new_loc = get_direction(loc, EAST);

    if (angle > 22.5 && angle < 67.5) new_loc = get_direction(loc, SOUTH_EAST);

    if (angle > 67.5 && angle < 113.5) new_loc = get_direction(loc, SOUTH);

    if (angle > 113.5 && angle < 158.5) new_loc = get_direction(loc, SOUTH_WEST);

    if (angle > 158.5 || angle < -157.5) new_loc = get_direction(loc, WEST);
