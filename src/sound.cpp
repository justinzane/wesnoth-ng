...
class sound_cache_chunk {
    public:
        sound_cache_chunk(const std::string& f) :
            group(sound::nullptr_CHANNEL), file(f), data_(nullptr) {
        }
...
static std::string pick_one(const std::string &files) {
    ...
    if (prev_choices.find(files) != prev_choices.end()) {
        ...
        if (choice >= prev_choices[files]) ++choice;
...
void reset_sound() {
    ...
    if (music || sound || bell || UI_sound) {
        ...
        if ( !music) sound::stop_music();
        if ( !sound) sound::stop_sound();
        if ( !UI_sound) sound::stop_UI_sound();
        if ( !bell) sound::stop_bell();
    }
}
...
void music_thinker::process(events::pump_info &info) {
    if (preferences::music_on()) {
        ...
        if (music_start_time && info.ticks( &music_refresh, music_refresh_rate)
            >= music_start_time - fadingout_time) {
            want_new_music = true;
        }
...
void commit_music_changes() {
    ...
    if (current_track.play_once()) return;
    ...
    if (current_track_list.empty()) return;
